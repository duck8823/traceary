...
func TestMemoryFamily_JSON_Goldens(t *testing.T) {
	...
	cases := []struct {
		...
		{"set-validity", []string{"memory", "admin", "set-validity", "--db-path", "/tmp/test-traceary.db", "--from", "2026-04-13T09:00:00Z", "--to", "2026-05-13T09:00:00Z", "--json", "memory-golden-accepted"}, "set-validity.golden.json"},
		{"extract", []string{"memory", "admin", "extract", "--db-path", "/tmp/test-traceary.db", "--session-id", "session-golden", "--json"}, "extract.golden.json"},
		{"import-codex", []string{"memory", "admin", "import", "codex", "--db-path", "/tmp/test-traceary.db", "--root", "/tmp/codex-memories", "--workspace", "github.com/duck8823/traceary", "--json"}, "import-codex.golden.json"},
		{"import-instructions", []string{"memory", "admin", "import", "instructions", "--db-path", "/tmp/test-traceary.db", "--source", "codex", "--in", "/tmp/AGENTS.md", "--workspace", "github.com/duck8823/traceary", "--json"}, "import-instructions.golden.json"},
		{"export", []string{"memory", "admin", "export", "--db-path", "/tmp/test-traceary.db", "--target", "codex", "--workspace", "github.com/duck8823/traceary", "--out", filepath.Join(t.TempDir(), "AGENTS.md"), "--json"}, "export.golden.json"},
