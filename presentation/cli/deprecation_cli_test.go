...
func TestRootCLI_RemovedV034NoReplacementSurfaces(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bundle import --orphan-edges", args: []string{"bundle", "import", "--in", "x.tbun", "--orphan-edges", "skip"}, wantErrHas: "unknown flag: --orphan-edges"},
	}
...
func TestRootCLI_MemoryDecayIsUnknownSubcommand(t *testing.T) {
	t.Setenv("TRACEARY_LANG", "en")
	tests := []struct {
		name       string
		args       []string
		...
		{name: "bare decay", args: []string{"memory", "decay"}, wantErrHas: `unknown subcommand "decay"`},
		{name: "decay apply", args: []string{"memory", "decay", "--apply"}, wantErrHas: "unknown flag: --apply"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stdout := &bytes.Buffer{}
...
func TestRootCLI_HooksGuideIsUnknownSubcommand(t *testing.T) {
	t.Setenv("TRACEARY_LANG", "en")
	tests := []struct {
		name       string
		args       []string
		...
		{name: "bare guide", args: []string{"hooks", "guide"}, wantErrHas: `unknown subcommand "guide"`},
		{name: "guide client", args: []string{"hooks", "guide", "--client", "claude"}, wantErrHas: "unknown flag: --client"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			stdout := &bytes.Buffer{}
