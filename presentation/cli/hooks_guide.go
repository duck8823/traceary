...
func buildHooksGuide(c *RootCLI, client string, projectDir string, outputPath string) (*hooksGuide, error) {
	...
	case "antigravity":
		notes = append(notes,
			...
			Localize("For non-interactive `agy --print`, merge the four exact allow rules from integrations/antigravity-plugin/permissions.example.json into Antigravity's `permissions` settings. Keep `--sandbox`; never replace them with a wildcard command grant, an unsandboxed grant, or `--dangerously-skip-permissions`.", "非対話の `agy --print` では、integrations/antigravity-plugin/permissions.example.json の 4 個の exact allow rule を Antigravity の `permissions` 設定に merge してください。`--sandbox` は維持し、command の wildcard grant、unsandboxed grant、`--dangerously-skip-permissions` で置き換えないでください。"),
