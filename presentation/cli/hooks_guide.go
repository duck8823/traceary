...
func buildHooksGuide(c *RootCLI, client string, projectDir string, outputPath string) (*hooksGuide, error) {
	...
	case "codex":
		notes = append(notes,
			Localize("Codex fires Stop after every assistant response, so Traceary records the turn transcript but keeps the session open. A Codex session ends only via `traceary session end` or stale GC (traceary session gc).", "Codex は assistant 応答ごとに Stop を fire するため、Traceary は turn の transcript を記録しつつ session は開いたままにします。Codex session は `traceary session end` または stale GC (traceary session gc) でのみ終了します。"),
		)
	case "gemini":
		notes = append(notes,
	...
	case "antigravity":
		notes = append(notes,
			Localize("Antigravity has no SessionStart hook: Traceary starts/refreshes the session idempotently from PreInvocation (keyed by conversationId). Like Codex, Stop is a per-execution boundary, so the session stays open and ends only via `traceary session end` or stale GC (traceary session gc).", "Antigravity には SessionStart hook がありません: Traceary は PreInvocation (conversationId 単位) から session を冪等に開始/更新します。Codex 同様 Stop は execution 単位の境界なので session は開いたままで、`traceary session end` または stale GC (traceary session gc) でのみ終了します。"),
