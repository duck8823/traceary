...
func buildHooksGuide(c *RootCLI, client string, projectDir string, outputPath string) (*hooksGuide, error) {
	...
	case "codex":
		notes = append(notes,
			Localize("Codex fires Stop after every assistant response, so Traceary records the turn transcript but keeps the session open. A Codex session ends only via `traceary session end` or stale GC (hook opportunistic GC / `traceary doctor --fix`).", "Codex は assistant 応答ごとに Stop を fire するため、Traceary は turn の transcript を記録しつつ session は開いたままにします。Codex session は `traceary session end` または stale GC（hook の opportunistic GC / `traceary doctor --fix`）でのみ終了します。"),
		)
	case "gemini":
		notes = append(notes,
	...
	case "antigravity":
		notes = append(notes,
			Localize("Antigravity has no SessionStart hook: Traceary starts/refreshes the session idempotently from PreInvocation (keyed by conversationId). Like Codex, Stop is a per-execution boundary, so the session stays open and ends only via `traceary session end` or stale GC (hook opportunistic GC / `traceary doctor --fix`).", "Antigravity には SessionStart hook がありません: Traceary は PreInvocation (conversationId 単位) から session を冪等に開始/更新します。Codex 同様 Stop は execution 単位の境界なので session は開いたままで、`traceary session end` または stale GC（hook の opportunistic GC / `traceary doctor --fix`）でのみ終了します。"),
			...
			Localize("For non-interactive `agy --print`, merge the four exact allow rules from integrations/antigravity-plugin/permissions.example.json into Antigravity CLI's global ~/.gemini/antigravity-cli/settings.json permissions settings. Gemini or project settings alone are not a headless permission source. Keep `--sandbox`; never replace them with a wildcard command grant, an unsandboxed grant, or `--dangerously-skip-permissions`.", "非対話の `agy --print` では、integrations/antigravity-plugin/permissions.example.json の 4 個の exact allow rule を Antigravity CLI の global ~/.gemini/antigravity-cli/settings.json の permissions 設定に merge してください。Gemini または project settings だけでは headless permission source になりません。`--sandbox` は維持し、command の wildcard grant、unsandboxed grant、`--dangerously-skip-permissions` で置き換えないでください。"),
			Localize("Register the Traceary hook group through exactly one route (workspace, user-level, or CLI plugin). Enabling more than one route can invoke the same lifecycle hook twice; doctor reports this as a warning.", "Traceary hook group は workspace、user-level、CLI plugin のいずれか 1 経路だけで登録してください。複数経路を有効にすると同じ lifecycle hook が 2 回呼ばれる可能性があり、doctor は警告を報告します。"),
