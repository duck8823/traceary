...
func buildHooksGuide(c *RootCLI, client string, projectDir string, outputPath string) (*hooksGuide, error) {
	...
	case "antigravity":
		notes = append(notes,
			...
			Localize("Headless `agy --print` captures session start (PreInvocation) and run_command audits only: print mode emits no Stop hook, so the final transcript/turn boundary is recorded only on interactive runs where the host emits Stop with transcriptPath. This is the expected capture level, not an install failure.", "headless な `agy --print` は session start (PreInvocation) と run_command audit のみを記録します: print mode は Stop hook を発行しないため、最終 transcript / turn 境界は host が transcriptPath 付き Stop を発行する interactive 実行でのみ記録されます。これは想定どおりの capture level であり、install 失敗ではありません。"),
