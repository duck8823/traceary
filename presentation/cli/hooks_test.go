...
func TestRootCLI_HooksPrintCommand(t *testing.T) {
	...
	t.Run("prints Codex hook settings", func(t *testing.T) {
		...
		// Body-free usage and transcript run before session-stop in the Stop entry:
		...
		if diff := cmp.Diff(`'/tmp/traceary bin/traceary' 'hook' 'usage' 'codex'`, settings.Hooks["Stop"][0].Hooks[0].Command); diff != "" {
			t.Fatalf("Stop[0] command mismatch (-want +got):\n%s", diff)
		}
		if diff := cmp.Diff(`'/tmp/traceary bin/traceary' 'hook' 'transcript' 'codex'`, settings.Hooks["Stop"][0].Hooks[1].Command); diff != "" {
			t.Fatalf("Stop[1] command mismatch (-want +got):\n%s", diff)
		}
		if diff := cmp.Diff(`'/tmp/traceary bin/traceary' 'hook' 'session' 'codex' 'stop'`, settings.Hooks["Stop"][0].Hooks[2].Command); diff != "" {
	...
	for _, client := range []string{"grok", "grok-build", "grok-cli"} {
		t.Run("prints native Grok hooks with "+client, func(t *testing.T) {
			rootCmd := newTestRootCLI().Command()
			stdout := &bytes.Buffer{}
			rootCmd.SetOut(stdout)
			rootCmd.SetErr(&bytes.Buffer{})
			...
			for event, action := range wantCommands {
				entries := settings.Hooks[event]
				if len(entries) != 1 || len(entries[0].Hooks) != 1 {
					t.Fatalf("Grok %s entries = %v, want one command", event, entries)
				}
				want := fmt.Sprintf(`'/tmp/traceary bin/traceary' 'hook' 'grok' '%s'`, action)
	...
	for _, client := range []string{"kimi", "kimi-code", "kimi-cli"} {
		t.Run("renders Kimi TOML hook rules with "+client, func(t *testing.T) {
			...
			text := stdout.String()
			for _, event := range []string{"SessionStart", "SessionEnd", "UserPromptSubmit", "PreToolUse", "PostToolUse", "PostToolUseFailure", "Stop", "SubagentStop", "PreCompact", "PostCompact"} {
				if !strings.Contains(text, `event = "`+event+`"`) {
					t.Errorf("Kimi TOML missing event %q:\n%s", event, text)
				}
			}
			if !strings.Contains(text, `matcher = "Agent"`) {
				t.Errorf("Kimi TOML missing the Agent matcher:\n%s", text)
			}
...
func TestRootCLI_HooksInstallCommand(t *testing.T) {
	...
	t.Run("installs native Grok hooks at the project path", func(t *testing.T) {
		for _, client := range []string{"grok", "grok-build", "grok-cli"} {
			t.Run(client, func(t *testing.T) {
				...
				outputPath := filepath.Join(clientProjectDir, ".grok", "hooks", "traceary.json")
