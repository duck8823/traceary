...
func (s *memoryExtractionMemoryUsecaseStub) AttachCandidateRefs(context.Context, domtypes.MemoryID, []domtypes.EvidenceRef, []domtypes.ArtifactRef) (apptypes.MemoryDetails, error) {
	return apptypes.MemoryDetails{}, nil
}
...
// routing: obvious code/diff fragments are dropped entirely (#1169), while the
// softer noise classes (standalone commands, review-only conclusions, work
// declarations, PR/Round chatter) are routed to the extracted-hidden source so
// the default `memory inbox list` view stays clean (#857).
func TestMemoryUsecase_Extract_HidesNoisyCandidates(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:        "generated code marker under structured label is dropped",
