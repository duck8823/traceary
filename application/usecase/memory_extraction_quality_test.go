...
func TestClassifyExtractionNoise(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "binary diff", fact: "Binary files a/foo and b/foo differ", want: []string{"diff_header"}},
		...
		// Durable prose that starts with a CLI flag or sign matches the content
		// pattern (diff_fragment) but must never become the droppable diff_header
		// (#1169 Codex finding): it stays hidden and recoverable.
		{name: "flag-prefixed durable constraint", fact: "-race must be enabled for Go tests", want: []string{"diff_fragment"}},
		{name: "plus-prefixed durable note", fact: "+1 for caching the build output", want: []string{"diff_fragment"}},
		// Prose that *starts with* a triple-dash / triple-plus / bare @@ / "Binary
		// files" token but is NOT in genuine unified-diff form must stay
		// diff_fragment (hidden, recoverable), never the droppable diff_header
		// (#1169 Codex round-5 finding). The droppable header requires real diff
		// structure (a/|b/|/dev/null paths, a full hunk range, or "... differ").
		{name: "triple-dash durable prose", fact: "--- separates YAML documents", want: []string{"diff_fragment"}},
		{name: "triple-plus durable prose", fact: "+++ marks added sections in the changelog", want: []string{"diff_fragment"}},
		{name: "bare at-at durable prose", fact: "@@ is the matrix-multiply operator in NumPy", want: []string{"diff_fragment"}},
		{name: "binary files durable prose", fact: "Binary files are stored in object storage, not git", want: []string{"diff_fragment"}},
		// "diff --git" / "Binary files ... differ" prose that lacks the real
		// a/ b/ (or /dev/null) path shape must stay diff_fragment (hidden),
		...
		{name: "git index line", fact: "index 1a2b3c4..5d6e7f8 100644", want: []string{"diff_header"}},
		{name: "binary diff dev-null", fact: "Binary files /dev/null and b/logo.png differ", want: []string{"diff_header"}},
		// A binary-diff-looking constraint that ends in "differ" but is not the
		// exact `<old> and <new> differ` shape must stay hidden, not drop
		// (#1169 Codex round-9 finding).
		{name: "binary files durable constraint ending differ", fact: "Binary files a/foo and b/foo must not differ", want: []string{"diff_fragment"}},
