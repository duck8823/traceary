...
// fragment that should not be created as a durable-memory candidate at all
// (#1169). Only diff fragments qualify: their detection is anchored to
// unified-diff line shapes (isDiffFragment), so the false-positive surface on
// durable prose is negligible. generated_code is deliberately excluded — its
// detection is a loose substring match ("auto-generated", "code generated by",
// "自動生成") that also fires on durable prose *about* generated files (e.g.
// "do not edit auto-generated files; regenerate them"). Those candidates stay
// hidden (recoverable via `memory inbox --include-hidden`) instead of being
// silently discarded. Explicit remember-intent is handled by the caller and
// always overrides this drop.
func isDroppableExtractionFragment(reasons []string) bool {
	for _, reason := range reasons {
		if reason == extractionNoiseDiffFragment {
			return true
		}
	}
	return false
...
// generated-code markers, which are surfaced as low-value noise even though
// they are kept (not dropped) because their substring matcher can misfire on
// durable prose. The count is non-destructive, so the occasional false positive
// only nudges a diagnostic number rather than discarding a memory.
