...
func validateBundleTableEntryFields(entry bundleTableEntry) error {
	if entry.File == "" {
		return xerrors.Errorf("bundle table %s has an empty file", entry.TableName)
	}
	if entry.Checksum == "" {
		return xerrors.Errorf("bundle table %s has an empty checksum", entry.TableName)
	}
	if entry.RowCount < 0 {
		return xerrors.Errorf("bundle table %s has a negative row_count", entry.TableName)
	}
	return nil
}
...
func manifestTableEntries(
	...
	case 2:
		...
		entries := make([]bundleTableEntry, 0, len(names)+1)
