...
func TestIsFragmentLikeNoise(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{name: "no reasons is not fragment-like", reasons: nil, want: false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := isFragmentLikeNoise(tc.reasons); got != tc.want {
				t.Fatalf("isFragmentLikeNoise(%v) = %v, want %v", tc.reasons, got, tc.want)
			}
		})
	}
}
...
func TestSummarizeCandidateHygiene(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "generated code marker candidate is fragment-like (counted, not dropped)",
			summaries: []apptypes.MemorySummary{
				candidate(t, "m1", "// Code generated by MockGen. DO NOT EDIT.", domtypes.MemorySourceExtracted, fresh),
			},
			want: apptypes.CandidateHygieneCounts{FragmentLike: 1},
		},
		{
