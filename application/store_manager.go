...
type StoreManager interface {
	...
	// CollectGarbage cleans up store records older than before for the selected
	// target. foldedBefore bounds the body discard to events whose covering
	// refinement already existed at that instant, so a caller that folds
	// immediately beforehand cannot discard what it just summarised.
	// Returns the count of affected records.
	CollectGarbage(ctx context.Context, before time.Time, foldedBefore time.Time, target apptypes.GarbageCollectionTarget, dryRun bool) (int, error)
