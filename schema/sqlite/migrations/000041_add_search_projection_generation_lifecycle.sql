CREATE TABLE search_projection_generation_lifecycle(
 ...
 state TEXT NOT NULL CHECK(state IN('rebuilding','complete','abandoned')),
...
SELECT generation_id,CASE WHEN state='complete' THEN 'complete' ELSE 'rebuilding' END,config_hash,source_revision,high_water
