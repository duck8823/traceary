...
-- New installs/upgrades inventory pre-existing canonical identities through
-- the bounded projection lifecycle. Migration 42 creates this marker with a
-- completed default for databases that already ran the historical v38 scan.
CREATE TABLE search_projection_inventory_compat(singleton INTEGER PRIMARY KEY CHECK(singleton=1),requires_inventory INTEGER NOT NULL CHECK(requires_inventory IN(0,1)));
INSERT INTO search_projection_inventory_compat VALUES(1,1);
...
CREATE TRIGGER search_projection_events_delete BEFORE DELETE ON events WHEN EXISTS(SELECT 1 FROM search_projection_state s WHERE s.state='rebuilding' AND (EXISTS(SELECT 1 FROM search_projection_inventory_compat c WHERE c.requires_inventory=1) OR EXISTS(SELECT 1 FROM search_projection_source_sequence q WHERE q.event_id=old.id AND q.sequence<=s.high_water))) BEGIN UPDATE search_projection_source_revision SET revision=revision+1; END;
...
CREATE TRIGGER search_projection_audits_delete AFTER DELETE ON command_audits WHEN EXISTS(SELECT 1 FROM search_projection_state s WHERE s.state='rebuilding' AND (EXISTS(SELECT 1 FROM search_projection_inventory_compat c WHERE c.requires_inventory=1) OR EXISTS(SELECT 1 FROM search_projection_source_sequence q WHERE q.event_id=old.event_id AND q.sequence<=s.high_water))) BEGIN UPDATE search_projection_source_revision SET revision=revision+1; END;
