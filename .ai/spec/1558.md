...
- **Phase A:** Run the existing opt-in direct metadata benchmark with eight
  ...
  the ordered plan must read only the projection, returned body bytes must be
  zero, 25 measurements are required, and p95 must remain below 250 ms.
...
| --- | --- | --- | --- |
| Synthetic extent | Eight generated, content-free 256 MiB SQLite values plus eight trigger-maintained projection rows | Makes page and stored-byte scale real without using user data | Managed and stored bytes are each at least 2 GiB; event/projection counts match; the measured plan is projection-only |
...
| --- | --- | --- | --- | --- |
| Phase-A gate | Opt-in eight-extent fixture | Direct metadata range is measured 25 times | managed/stored bytes >= 2 GiB, event/projection rows=8, projection-only plan, returned body bytes=0, p95 <250 ms |
...
- Phase A recorded 2,418,774,016 managed bytes, 2,147,483,648 stored-body
  ...
  ordered plan, zero returned body bytes, and 25 measurements.
- Phase-A p95 was 0.073834 ms against the unchanged 250 ms target.
...
- Phases C through E completed without changing their contracts. Phase-C
  latencies remain observations only.
