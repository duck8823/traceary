...
"$binary" doctor --db-path "$db_path" --json --warnings-ok >"$report_path"
...
identity = report["workspace_identity"]
exact = identity["exact_delivery"]
expected_rate = 2 / 204
checks = [
    (before == after, f"doctor mutated event counts: {before} -> {after}"),
