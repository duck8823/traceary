...
| 目的 | コマンド | 使いどころ |
|---|---|---|
| operator cockpit から始める | `traceary`（明示するなら `traceary tui`） | 新着 event / candidate memory に気付き、live tail / doctor details / memory review へ移動 |
...
対話 terminal では `traceary` が Tail-first operator cockpit を開きます。`traceary tui` は同じ cockpit を明示的に開く互換 entrypoint として残ります。cockpit は TTY 専用の surface で、新着 event、candidate memory、doctor warning、直近の失敗に気付き、そこから live tail、doctor details、memory review へ移動できます。非対話 shell では bare `traceary` は deterministic な help / fallback guidance を表示するため、script からは `traceary list`、`traceary top --snapshot [--json]`、`traceary doctor --json` などの明示的な command を使ってください。
