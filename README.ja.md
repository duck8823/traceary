...
インストール後、対話 terminal で `traceary` を実行すると Tail-first TUI が開きます。script、pipe、CI など TTY がない場所では `traceary list`、`traceary top --snapshot [--json]`、`traceary doctor --json` などの script-friendly subcommand を直接呼んでください。同じ cockpit を明示的に開く互換 entrypoint として `traceary tui` も引き続き使えます。
...
`memory inbox review` は共通 Bubble Tea TUI の上に乗った TTY 専用のウォークスルーです。画面内のキー操作は `a` accept / `x` reject / `s` skip / `e` edit/distill / `v` evidence 表示 / `?` help / `q` quit です。edit/distill は LLM が書いたメモリ候補を自動で accept しません。`traceary memory store distill` 経由で記録し、operator 自身に新しい fact を入力させる作りです。非 TTY で起動した場合はエラー終了（exit code `2`）し、batch 用の fallback として `memory inbox list` + `memory inbox accept|reject`（v0.14 から positional id と `--id-only` も使えます）を案内します。
...
| 目的 | コマンド | 使いどころ |
|---|---|---|
| operator cockpit から始める | `traceary`（明示するなら `traceary tui`） | 新着 event / メモリ候補に気付き、live tail / doctor details / memory review へ移動 |
| workspace dashboard を見る | `traceary top` | active session、直近の失敗 / command、メモリ候補、stale memory を 1 つの TUI で確認 |
...
対話 terminal では `traceary` が Tail-first operator cockpit を開きます。`traceary tui` は同じ cockpit を明示的に開く互換 entrypoint として残ります。cockpit は TTY 専用の surface で、新着 event、メモリ候補、doctor warning、直近の失敗に気付き、そこから live tail、doctor details、memory review へ移動できます。非対話 shell では bare `traceary` は deterministic な help / fallback guidance を表示するため、script からは `traceary list`、`traceary top --snapshot [--json]`、`traceary doctor --json` などの明示的な command を使ってください。
