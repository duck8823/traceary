...
| Durable memory | decision / constraint / preference / artifact ref など | `traceary-memory-review` SKILL（review 意図 trigger）と `traceary-memory-remember` SKILL（明示 write trigger）で curate。旧 `traceary-memory-capture` SKILL は v0.11.0 で deprecated（v0.12 で削除） |
