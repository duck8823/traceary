...
| 目的 | コマンド | 使いどころ |
|---|---|---|
| operator cockpit から始める | `traceary tui` | 新着 event / candidate memory に気付き、live tail / doctor details / memory review へ移動 |
...
`tui` は operator cockpit を開きます。TTY 専用の home 画面で、新着 event、candidate memory、doctor warning、直近の失敗に気付き、そこから live tail、doctor details、memory review へ移動できます。v0.17.0 では bare `traceary` の挙動は変更せず、従来どおり help / usage を表示します。安定した明示 entrypoint として `traceary tui` を使ってください。
