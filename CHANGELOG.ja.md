...
### Changed
- **全文検索が実際に被覆する範囲を報告 (#1838)** — `store search-projection status` は 30 以上のフィールドを報告しますが、`traceary search` が全文で answer できる期間を示すものは 1 つもありませんでした。`recent_cutoff_norm` は `Start` 時点で導出する source phase の prefilter 境界であって eviction 後に残った範囲ではなく、実際の store では空です。追加フィールド `recent_oldest_norm` / `recent_newest_norm` が、recent ティアに実際に保持されている最古・最新の時刻を報告します。読み取り経路と同じ条件（complete な世代であり active 世代として指名されていること）を適用するため、検索がプロジェクションを読んでいない間（再構築中、drift 後、最初の世代が complete する前）は、検索が参照しない行の範囲を示すのではなく省略します。これは全文検索の範囲のみを示し、それより古いイベントも session ティア経由で検索できます。
