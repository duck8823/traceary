...
### Added
- **メタデータ限定の容量診断と copy 上の benchmark (#1607, #1624)** — `traceary store capacity` は集計したページ、オブジェクト、payload サイズ区分、空きページ、WAL の測定値を versioned な `traceary.capacity/v1` JSON で出力します。保存値や識別子は読み出しも出力もせず、SQLite の `dbstat` が使えない場合はオブジェクト内訳を省略します。benchmark command は一貫した immutable copy を対象に、production の active/latest/handoff/search 経路の上限付き p50/p95 timing と query plan を出力し、timeout は実測した下限として記録します。growth guidance と warning も同じメタデータ限定経路を使い、retention や index の設定予算を filesystem の上限だとは主張しません。
...
- **`store gc` が破棄前に未被覆の履歴を consolidation (#1674, #1676, #1683, #1719)** — refinement が被覆していない event range を上限付きの degraded mechanical summary に折り畳んでから、pass 開始時点の coverage に対して discard を実行します。対応する stop hook を持つ host では、未精製の素材が設定した pressure threshold を超えたときに consolidation を依頼します。gc 経路は上限付きの fallback として残ります。discovery は range 数と wall time の上限を持ち、複数回の実行で進捗を引き継ぎます。読めない range 一つは件数に含めて skip し、全体を abort しません。残りの range を表示し、未要約の body を無言で破棄しません。
