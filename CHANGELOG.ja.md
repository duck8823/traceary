...
- **Bundle durable memories (#739)** — bundle export は scope、validity、supersession、evidence refs、artifact refs を含む `memories.ndjson` を追加します。bundle import は `memories_imported` / `memories_skipped` を表示し、新規 import された memory は source が accepted でも review 必須の `candidate`（proposed trust default）として保存するため、cross-machine の memory trust が自動昇格しません。
