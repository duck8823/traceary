...
### Fixed
- **projection 再構築がインデックスファミリ予算を大きく超えてピークに達しなくなりました (#1796)** — eviction が source 走査の完了を待たず、走査と交互に実行されるようになりました。これにより導出されたソーステキスト上限が、事後合計ではなくピークを上限します。各バッチは永続化された `recent_source_bytes` カウンタを上限と比較し、追加投入の前にその世代の最古のドキュメントから evict するため、プロセス再起動をまたいでも上限が維持されます。従来は最後のソース行を走査し終えるまで投入分をすべて保持しており、実測では走査を 64% 残した時点で 1.43 GiB の上限に対し 3.74 GiB に達していました。マイグレーション `000058` はこのカウンタを追加し、旧バイナリが再構築途中で残した世代を再開始します。予算が何を上限し何を上限しないか（退避可能な recent ティアとコーパス比例のセッション／フィンガープリントティアの違い、再構築中に 2 世代を保持すること）は `docs/search-projection-rebuild.ja.md` に明記しました。
