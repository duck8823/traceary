...
### Changed
- **未使用の literal fingerprint candidate index を削除 (#1808)** — migration `000059` が `literal_search_fingerprints` テーブルの冗長な 118,489,088 バイトの index を削除します。2 つの fingerprint reader subquery の primary key による query plan は変わらないため、generation は無効化しません。SQLite が解放されたページを store 内で再利用しますが、`traceary store compact` / `VACUUM` を実行するまで filesystem には返却されません。
