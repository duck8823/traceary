...
- **search projection の lifecycle と tiered search を明示的な上限付き workflow に (#1611, #1612, #1622, #1634, #1639, #1679, #1680, #1733, #1735, #1740, #1793, #1794, #1798, #1801, #1810)** — store open は一度に上限付きの generation 作業を進め、`traceary store search-projection start|resume|status|abort` は durable checkpoint、過去 source inventory、session-tier readiness、exclusion、failure class、capacity evidence を公開します。`traceary search` は literal decode を authority とし、cutover 後の tail を統合し、event match と session summary/keyword match を別々に公開します。complete な generation では fingerprint pre-filter と session tier が使えますが、incomplete、failed、drifted な generation は検証されていない session result を返さず明示されます。candidate budget を使い切った場合は partial page ではなく `index_incomplete` を報告します。`--index-family-bytes` は recent tier の eviction に使う物理 index-family の target であり、index bytes や rebuild peak の上限ではありません。generation 単位の budget を超える行は無限に再試行せず除外して報告します。退役した migration-032 index は fallback になりません。
