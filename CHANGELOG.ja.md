...
### Added
- **メタデータ限定の容量診断 (#1607, #1624)** — `traceary store capacity` は集計したページ、オブジェクト、payload サイズ区分、空きページ、WAL の測定値を versioned な `traceary.capacity/v1` JSON で出力します。保存値や識別子は読み出しも出力もせず、SQLite の `dbstat` が使えない場合はオブジェクト内訳を省略します。growth guidance と warning も同じメタデータ限定経路を使い、retention や index の設定予算を filesystem の上限だとは主張しません。（production の query 経路を immutable copy 上で計測する `store-benchmark` harness は repository 内のツールであり、リリースバイナリには含まれません。）
- **copy 上で再開可能な payload-compression rehearsal (#1609, #1610, #1635, #1641, #1643, #1645, #1749)** — `traceary store payload-rehearsal preview|run|resume|scrub|rollback` は live/copy の identity、migration readiness、WAL と lock の上限を検査し、copy 上の versioned zstd shadow row だけを書き込み、集計 metrics と opaque hash を返します。run は commit 済み batch の境界でだけ停止し、同一 configuration で再開します。migration、scrub、rollback、WAL reservation は上限付きで、text の budget は文字数ではなく UTF-8 bytes で測定します。live の canonical event と audit row はこの workflow で activate も変更もされず、alias は拒否され、copy target には検証済みの物理 rollback artifact が必要です。
- **安全で再開可能な物理 store compaction (#1613, #1633, #1645)** — `traceary store compact plan|apply|resume|status|rollback` は in-place vacuuming を、preview-first の run journal、上限付き candidate 検証、同一 filesystem 内の atomic exchange、保持される source-size rollback copy に置き換えます。unsafe な identity、sidecar、legacy index、compatibility、空き容量の状態は推測せず拒否し、中断時は `resume` で復旧し、検証済み run は `rollback` で戻せます。これは自動 GC ではなく物理 reclamation であり、operator が受け入れて削除するまで rollback artifact は成功後も残ります。
