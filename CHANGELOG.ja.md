...
### Removed
- **`traceary store search-projection` を削除 (#2077)** — start/resume/status/abort/probe は unknown subcommand（非ゼロ、`DEPRECATED` なし）。parked 復旧は `doctor --fix`（必要なら start、続けて bounded resume）。新しい世代、または進行中 rebuild の lock-time resume は `store compact --projection-rebuild`（同じ budget flag。`--index-family-bytes` を含む）。abort は `store compact --projection-abort`。status と予算判定は `doctor`（`search-projection-parked` / `search-projection-budget`）。大きい store の既定 doctor はこれまでどおり SQLite を開かず dbstat も歩かない。
