...
- **`traceary search --kind` が SESSIONS グループを空にしたことを伝えるようになりました (#1728)** — セッション要約とキーワードは kind 別の内訳を持たないため、`--kind` 指定時のセッションティアは絞り込まない結果を返すのではなく fail closed します。それを無言で行わなくなりました。`--kind` が指定されているときだけ、kind filter を外して同じセッション検索をもう一度実行し、一致があればその件数と理由を stderr に報告します。追加のクエリが走るのは `--kind` 指定時のみです。セッション行に対する filter の意味（`--from` / `--to` はセッションの開始時刻、`--failures` はセッション内のいずれかの失敗コマンド）は `docs/cli/README.ja.md` に明記しました。挙動自体は変更していません。
