...
### Changed
- **残っていた互換 surface を削除する（#2322）。** offline migration 86 は非 NULL 値を拒否したあと `event_metadata_projection.legacy_source_hook`（と CHECK）を DROP し、`event_metadata_projection_events_after_insert` と `event_metadata_projection_events_after_update` を列なしで作り直し、`minimum_reader_version` を 42 に上げる。initialize が既に保証する対象への runtime capability probe は削除する（event-delivery の `hook_delivery_attempts` / `session_workspace_observations` テーブル skip、workspace identity catch-up、attestation codec）。`observation_count` の write-path probe は残す（076 は data-dependent offline のため、`doctor --fix` 前の live open は collapse 前の行形で書く）。canonical event/audit codec probe は 082 の source 読み取り経路に残す（適用前の source にはまだ `body_codec` がある）。`session_workspace_aliases` は残す（named reader がまだ必要）。migration-032 search-index retire 経路は doctor caller 付きで残す。`store compact` の吸収 flag は #2326 で既に削除済み。残 alias flag はそれぞれ理由付き。欠ける `store_format_state` は pre-36 lineage（と空 bootstrap）以外 fail-closed。modern ledger や format state のない外部 `events` ファイルは拒否する。`maximum_payload_format` は #2323 で既に削除済み。DROP COLUMN はページを freelist へ移すだけで、VACUUM / candidate rewrite なしではファイルは縮まない。親 #2315 は open のまま。
- **memory graph を削除する（#2327）。** domain model、usecase、datasource、GC target `memory_edges`、bundle export writer はなくなる。offline migration 85 は空の `memory_edges` を #2328 candidate 上で DROP し、`minimum_reader_version` を 41 に上げ、非空テーブルは件数と 0.48.2 の `bundle export` 手順を出して拒否する。新しい bundle export は当該 entry を出さない（manifest は v2 のまま。減算であり version bump ではない）。旧ゼロ行 `memory_edges` は checksum と件数検証のあと skip、非ゼロは何も import せず原子的に拒否する。`bundle import --orphan-edges` は unknown flag。残 edge の取り出しは Traceary 0.48.2 に pin する。この binary に converter はない。DROP はページを freelist へ移すだけで、VACUUM / candidate rewrite なしではファイルは縮まない。#2321 へ引き継ぎ、親 #2315 は open のまま。
- **archive package と retention-plan surface を削除する（#2326）。** `store compact --archive` / `--archive-verify` / `--archive-restore`、20 個の archive/retention flag、`hook archive_auto`、`config.json` の `retention.mode=archive_then_gc`、file-retention domain layer はなくなる（hidden no-op ではなく unknown）。offline migration 84 は空の `archive_segments` を #2328 candidate 上で DROP し、`minimum_reader_version` を 40 に上げ、非空テーブルは件数と 0.48.2 の restore/export 手順を出して拒否する。既存 archive package の取り出しは Traceary 0.48.2 に pin する。この binary に converter / 互換 reader はない。`bundle` と `store backup` は変更しない（backup は retention sidecar を書かない）。DROP はページを freelist へ移すだけで、VACUUM / candidate rewrite なしではファイルは縮まない。親 #2315 は open のまま。
