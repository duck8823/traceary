...
### Fixed
- **`sessions --snapshot` が大きな store でも返るようになりました (#1863)** — lineage 展開が、ひとつの家系をその構成員の数だけ繰り返し問い合わせていました。`LineageOf` は最上位の祖先を根とする家系全体を返し、`scanSessionSummary` は返された各行の latest event body を hydrate します。`expandSessionLineages` の重複排除はその後だったため、active な構成員が k 人の家系は k 回問い合わされて k 回 hydrate され、k-1 回分は捨てられていました。祖先をたどる再帰の深さ上限により、同じ家系の構成員でも `LineageOf` の返す集合が異なっていました。この上限を取り除くことで同一家系の不変条件が初めて成立し（これまで成立していませんでした）、展開時に先行する家系で既に収集済みの session を安全に skip できます。32.45 GB・session 27,540 件（うち active 485 件）の store での実測: `--limit 100` は変更前 32.2 秒、変更後 26.3 秒、`--limit 200` は変更前 642.7 秒、変更後 98.9 秒、既定の snapshot は変更前は 30 分で返らず、変更後は 201 秒で完了しました。`--limit 100` の出力は変更前後でバイト単位で同一でした。この変更後に残るコストは #1865 で追跡します。
