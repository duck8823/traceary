...
### Fixed
- **`sessions --snapshot` が大きな store でも返るようになりました (#1863)** — lineage 展開が、ひとつの家系をその構成員の数だけ繰り返し問い合わせていました。`LineageOf` は最上位の祖先を根とする家系全体を返し、`scanSessionSummary` は返された各行の latest event body を hydrate します。`expandSessionLineages` の重複排除はその後だったため、active な構成員が k 人の家系は k 回問い合わされて k 回 hydrate され、k-1 回分は捨てられていました。展開時に、先行する家系で既に収集済みの session を skip するようにしました。ある session が別の session の家系に現れるなら根も家系も同じであるため、出力は変わりません。32.45 GB・session 27,540 件（うち active 485 件）の store での実測: 既定の snapshot は変更前は 30 分で返らず、変更後は 201 秒で完了。`--limit 200` では 642.7 秒から 98.9 秒。`--limit 100` の出力は変更前後で完全一致しました。残るコストは active session 数に比例し、1 件あたり約 0.25 秒です (#1865)。
