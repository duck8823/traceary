...
### Fixed
- **`sessions --snapshot` が大きな store でも返るようになりました (#1863)** — lineage 展開が、ひとつの家系をその構成員の数だけ繰り返し問い合わせていました。`LineageOf` は家系のすべての構成員を同じ root に解決し、その結果として同じ lineage 集合を返します。`scanSessionSummary` は返された各行の latest event body を hydrate し、`expandSessionLineages` の重複排除はその後だったため、active な構成員が k 人の家系は k 回問い合わされて k 回 hydrate され、k-1 回分は捨てられていました。同一家系の不変条件は、祖先をたどる再帰の深さ上限と、comma で衝突しうる raw ID の path guard の両方によって壊れていました。深さ上限を取り除き、path の ID を hex 化することでこの不変条件を成立させ、展開時に先行する家系で既に収集済みの session を安全に skip できます。返される lineage 集合には descendant の深さ上限が残り、完全性の制限は #1867 で追跡します。32.45 GB・session 27,540 件（うち active 485 件）の store での実測: `--limit 100` は変更前 32.2 秒、変更後 26.3 秒、`--limit 200` は変更前 642.7 秒、変更後 98.9 秒、既定の snapshot は変更前は 30 分で返らず、変更後は 201 秒で完了しました。`--limit 100` の出力は変更前後でバイト単位で同一でした。この変更後に残るコストは #1865 で追跡します。
