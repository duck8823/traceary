...
### Removed
- **耐久 search index はなくなりました。** phrase search が動きます。旧 index では扱えなかったハイフン付き語も含みます。結果は session refinement を先に、raw record の unindexed scan を次に見ます。大きな store では 1〜2 秒かかることがあります（#2318, #2319）。
- **archive package と retention-plan 機能はなくなりました（#2326）。** `store compact --archive` / `--archive-verify` / `--archive-restore`、20 個の archive/retention flag、`hook archive_auto`、`config.json` の `retention.mode=archive_then_gc` は hidden no-op ではなく unknown です。既存 archive package の取り出しは Traceary 0.48.2 です。この binary に converter はありません。
- **memory graph はなくなりました（#2327）。** domain model、usecase、datasource、GC target `memory_edges`、bundle export writer は削除です。`bundle import --orphan-edges` は unknown flag です。残 edge の取り出しは 0.48.2 に pin します。
- **degraded refinement はなくなりました（#2325）。** orphan consolidation が機械的に作っていた経路は discard 機構ごと削除され、refinement は agent が書いたものだけです。
- **`store compact` から消えた flag**: `--projection-rebuild`、`--projection-abort`、`--index-family-bytes`、`--decoded-bytes`、`--recent-age`、`--lock-time`（#2319, #2320）。残る compact surface は `--db-path` と `--work-dir` です。
- **usage report から lineage 帰属が消えます（#2317）。** repository、ticket、pull-request 番号、batch id、packet bytes、tool-output bytes は usage report から消えます。observation 本体と host / run id は残ります。
- **新しい bundle は `run_lineages` と `memory_edges` entry を出しません。** ゼロ行の legacy entry は import できます。行があるものは件数と 0.48.2 の取り出し手順を出して原子的に拒否します。
- **残っていた互換 surface を削除します（#2322）。** offline migration 86 は非 NULL 値を拒否したあと `event_metadata_projection.legacy_source_hook`（と CHECK）を DROP し、両方の trigger を列なしで作り直し、`minimum_reader_version` を 42 に上げます。initialize が既に保証する対象への runtime capability probe は削除です。`maximum_payload_format` は #2323 で既に削除済みです。

### Changed
- **store が縮むのは明示的な upgrade を通したときだけです。** DROP はページを freelist へ移します。ファイル自体が縮むのは candidate が rewrite されて publish されたときです。通常の open はこの作業を行いません。pending を報告して戻ります。リリース PR のサイズは operator store の copy で測った値です（freelist 回収と物理ファイルサイズを分け、payload codec 削除による増加も含む）。約束の数字ではありません。
- **live store から行を消すには明示的な bound approval が必要です。** 残っているデータを壊す削除では `traceary doctor --fix` は止まり、失われる内容を出します。その store / run / set digest に対する `--approve-drop N:<hex>` および/または `--approve-unavailable-retention M:<hex>` のあとだけ進みます。
- **downgrade はサポートしません。** `minimum_reader_version` が上がり、古い binary は誤読せず store を拒否します。
- **search-projection サブシステムを削除します（#2319）。** offline migration 80 が 13 表と未読の `search_projection_recent_fts` を DROP し、`minimum_reader_version` を 36 に上げます。適用は #2328 の verified candidate 上で、store open では走りません。
- **report usage は repository/ticket/PR/batch 帰属と packet/tool byte のために `run_lineages` を結合しません（#2317）。** 集計キーは `{provider,engine,model}` と `{engine}`。observation の `run_host`/`run_id` は残します。
- **forward migration 079 は `usage_observation_runs` を FK なしで rebuild したあと `run_lineages` を DROP し、`minimum_reader_version` を 35 に上げます（#2317）。** 空 store は inline bootstrap。非空 store は #2328 candidate 上で `traceary doctor --fix`。残行は `--approve-drop N:<hex>` なしでは止まります。
- **archive package と retention-plan surface を削除します（#2326）。** offline migration 84 は空の `archive_segments` を #2328 candidate 上で DROP し、`minimum_reader_version` を 40 に上げ、非空テーブルは件数と 0.48.2 の restore/export 手順を出して拒否します。
- **memory graph を削除します（#2327）。** offline migration 85 は空の `memory_edges` を #2328 candidate 上で DROP し、`minimum_reader_version` を 41 に上げ、非空テーブルは件数と 0.48.2 の `bundle export` 手順を出して拒否します。
...
- **読み取り専用ツールの audit は出力本文ではなくアクセス metadata を保存する（#2267）。** 分類済みの成功した file-read / list / grep（Claude `Read`/`Grep`/`Glob`/`NotebookRead`/`WebFetch`、Grok `read_file`/`grep`/`list_dir`、Kimi `Read`/`Grep`/`Glob`/`ReadMediaFile`）は `{paths, bytes, sha256, truncated}` を `command_audits.output_metadata` に残し、`output_text` は空にする。mutating ツール、shell、MCP、失敗・denied の読み取り、手動の `traceary audit` は従来どおり全文を保存する。Codex の read ツールは未確認のため allowlist に入れない。Gemini の名前は前方互換のために分類するが、現行 AfterTool matcher では届かない。`audit.max_output_bytes` は成功した読み取り専用ツールには効かない。履歴行は書き換えない。archive v1 セグメントは新しい列を落とす。親 #2263 は open のまま。

### Fixed
- **Stop-hook の consolidation 依頼はセッションあたり 1 回で、`stop_hook_active` を尊重する（#2275）。** payload に `stop_hook_active: true` がある Stop — hook 自身が引き起こした継続 — は exit 2 にせず、open な `consolidation_requests` 行があるセッションは refinement が stamp するまで再依頼しない（#2272 は未 refine セッションあたり平均 4.8、最大 31 の追加 Stop を計測）。reason は `traceary-session-refine` skill 名と、実行可能な `traceary session refine <id> --covers-to <event-id> --summary "…" --produced-by agent` 1 行を載せ、ledger が記録する latest event id と同じ id を解決する。Kimi の payload 正規化は `stop_hook_active` を残すので native の `hook kimi stop` 経路も対象になる。ledger の失敗はすべて fail-open で依頼は届ける。ホスト向けコメントと `consolidation.threshold_bytes` の doc 行は実メカニズム（3 ホストとも exit 2 + stderr、`decision` envelope なし）と実除外（Gemini, Antigravity, Grok）を述べる。スキーマ変更なし。親 #2263 は open のまま。
...
### Docs
- **Markdown と landing HTML を現行 CLI と 2026-08-21 probe に揃えた (#2239)** — 検証コマンドは `traceary list --kind …` / `--failures`（`list events` サブコマンドは存在しない）。host-coverage matrix の Verification 列は `matrix.json` から再生成。Grok/Kimi の probe note は plugin 0.46.0 を示す。landing の matrix は Grok 1.0.5 の cell を available（plugin hook 非 dispatch）とし、Kimi の未観測 SessionEnd の記載を落とす。integration ガイドは Grok/Kimi の hook 群を live 検証済みではなく「宣言」と呼ぶ。`doctor --fix` の spool drain は共有 45 秒 wall 配下の 200 件 round として記載。runtime や doctor の挙動は変更なし。親 #2230 は open のまま。
...
### Removed
- **`traceary hooks guide` (#2125)** — unknown subcommand（非ゼロ、`DEPRECATED` なし。owner 例外は #2108）。`hooks install --dry-run` は stderr に `想定 config path: <resolved>` を出し、stdout は貼り付け可能な config のまま。client 注記は `docs/hooks/README.ja.md` に残す。
- **`traceary report workspace-identity` (#2124)** — unknown subcommand（非ゼロ、`DEPRECATED` なし）。`doctor --json` に `workspace_identity` ブロックを足す（coverage、conflict pair、sources、samples、aliases、導出 `exact_delivery`）。text doctor は `workspace-aliases` の hint 以外そのまま。2 GiB 以上の既定 doctor は SQLite identity 照会を skip する。`report` 本体は残る。任意の `--include-heuristic` 本文スキャンは leaf と一緒に消える。
...
### Removed
- **`traceary store search-projection` を削除 (#2077)** — start/resume/status/abort/probe は unknown subcommand（非ゼロ、`DEPRECATED` なし）。parked した failed の復旧は `doctor --fix`（必要なら start、続けて bounded resume）。`doctor --fix` は健全な進行中 rebuild を駆動しない。新しい世代、hash が一致する進行中 resume（`rebuilding` または drifted/cleanup）、または hash 不一致の置き換えは `store compact --projection-rebuild`（同じ budget flag。`--index-family-bytes` を含む）。abort は `store compact --projection-abort`。start/置き換えは generation JSON（`generation_id`）、一致 resume は run-result JSON（`batches` / `progress` / `stop_reason`）を出す。status と予算判定は `doctor`（`search-projection-parked` / `search-projection-budget`）。大きい store の既定 doctor はこれまでどおり SQLite を開かず dbstat も歩かない。
- **`traceary replay` を削除 (#2078)** — unknown command（非ゼロ、`DEPRECATED` なし）。期間の読み取りは `report` / `context` / `list`。機械可搬 export は `bundle export`。以前の cli-stability 再保持と撤回済み #2060 を上書きします。
...
### Changed
- **セッション要約の正本は refinement であり `sessions.summary` ではない (#1706)** — list / tree / lineage / handoff は `session_refinements.summary` を読む。`session end --summary` は refinement を書く。`SetSummaryIfEmpty` は削除。`sessions.summary` 列は残す（DROP は data-dependent offline で、populated store の暗黙 migrate は拒否される）。
- **`store compact` が重複した `command_executed` body を回収する (#1853)** — `command_audits` 行がある履歴 `events.body` を work copy 上で空にする（`EXISTS` ガード、空 identity SHA-256）。audit が無い `traceary log --kind command_executed` の body は残す。JSON の `released_command_body_rows` / `released_command_body_bytes` は stored blob の合計であり plaintext ではない。新しいコマンドは追加しない。`docs/research/command-executed-body-reclaim.ja.md` を参照。
- **暗黙の store open は data-dependent migration を拒否する (#1852)** — `migrate()` が `MigrationExecutionClass` を読む。offline（035, 045）は events があるストアでは適用せず、エラーが version と `traceary store init` を出す。空ストアは暗黙のまま現行 schema に届く。041 / 042 は `constant_in_place`。`docs/research/offline-migration-gate.ja.md` を参照。
- **search projection は lock 待ちと行作業を分ける (#1833)** — `BEGIN IMMEDIATE` に独自の取得予算がある。hold 中に超過した source 書き込み 1 件だけが `row_work` 除外になる（#1794 と同じ形）。競合では除外しない。`LockTime` は `ConfigHash` に入らない。`docs/research/search-projection-lock-vs-row-work.ja.md` を参照。
- **`recent_source_bytes` は報告する cache (#1819)** — `store search-projection status` が `recent_source_bytes`、`recent_source_bytes_measured`、`recent_source_bytes_delta` と evidence を出す。SUM は rebuild 中の `generation_id` であり `recent_bytes`（active generation）ではない。status は cache を書き換えず、trigger も足さない。`docs/research/recent-source-bytes-verifier.ja.md` を参照。
...
- **import / restore の writer は canonical payload codec を使う (#1779)** — bundle import、archive restore、dedupe restore、raw-body recovery は `encodeCanonicalPayload`（縮むとき zstd）。bundle / archive の交換形式は plaintext のまま。retention marker は identity のまま（apply/verify が stored TEXT を sentinel と比較するため）。`docs/research/payload-codec-call-sites.ja.md` を参照。
...
- **`traceary session tree` と `traceary session lineage` を削除 (#1869)** — スタンドアロンの session-graph browsing コマンドを削除しました。呼び出しは unknown subcommand（`unknown subcommand "tree"` / `"lineage"`）として非ゼロ終了し、`DEPRECATED` 通知は出しません。`traceary sessions --snapshot` / `--snapshot --json`（および bare の `sessions`）は script 向けの active-session view として残しています。
- **`traceary top` を削除 (#1690)** — v0.34 の非推奨期間 (#1688) を経て、互換 alias の `top` を削除しました。呼び出しは unknown command として非ゼロ終了し、`DEPRECATED` 通知は出しません。`traceary sessions`（または `sessions --snapshot` / `--snapshot --json`）を使ってください。snapshot 契約、`--snapshot` なしの `--json` エラー、`--profile ai` が `--snapshot --json` を要求する制約は変更していません。
- **Sessions live dashboard を削除 (#1766)** — v0.34 の非推奨期間 (#1765) を経て、対話 multi-pane の `traceary sessions` dashboard を削除しました。bare の `traceary sessions` は plain text command になり、どの caller でも `traceary sessions --snapshot` とバイト単位で同一です。`sessions --snapshot` / `--snapshot --json`、`--snapshot` なしの `--json` エラー、`--profile ai` が `--snapshot --json` を要求する制約は変更していません。共有の `presentation/cli/tui/` は `memory inbox review` 用に残しています。
...
### Added
- **メタデータ限定の容量診断 (#1607, #1624)** — `traceary store capacity` は集計したページ、オブジェクト、payload サイズ区分、空きページ、WAL の測定値を versioned な `traceary.capacity/v1` JSON で出力します。payload の内容や row identifier は読み出しも出力もせず、SQLite の `dbstat` が使えない場合はオブジェクト内訳を省略します。growth guidance と warning も同じメタデータ限定経路を使い、retention や index の設定予算を filesystem の上限だとは主張しません。（production の query 経路を immutable copy 上で計測する `store-benchmark` harness は repository 内のツールであり、リリースバイナリには含まれません。）
- **copy 上で再開可能な payload-compression rehearsal (#1609, #1610, #1635, #1641, #1643, #1645, #1749)** — `traceary store payload-rehearsal preview|run|resume|scrub|rollback` は live/copy の identity、migration readiness、WAL と lock の上限を検査し、copied target 上の versioned zstd shadow row に encoded payload bytes だけを書き込み、集計 metrics と opaque hash を返します。run は commit 済み batch の境界でだけ停止し、同一 configuration で再開します。migration、scrub、WAL reservation は上限付きです。publish lock limit は prepared target の exchange の前に deadline を設定しますが、開始後の exchange と `fsync` はそれを参照しません。`rollback` 全体としては上限がありません。verification、ファイル全体の restore、restore 後の検査はいずれも artifact 全体を走査し、設定可能な cap の対象外です（#1858）。text の budget は文字数ではなく UTF-8 bytes で測定します。live の canonical event と audit row はこの workflow で activate も変更もされず、alias は拒否され、copy target には検証済みの物理 rollback artifact が必要です。
- **安全で再開可能な物理 store compaction (#1613, #1633, #1645)** — `traceary store compact plan|apply|resume|status|rollback` は in-place vacuuming を、preview-first の run journal、設定された wall-time または resource の上限を持たない全件走査の candidate 検証、同一 filesystem 内の atomic exchange、保持される source-size rollback copy に置き換えます。unsafe な identity、sidecar、legacy index、compatibility、空き容量の状態は推測せず拒否し、中断時は `resume` で復旧し、検証済み run は `rollback` で戻せます。これは自動 GC ではなく物理 reclamation であり、operator が受け入れて削除するまで rollback artifact は成功後も残ります。
- **session refinement と compact digest の handoff (#1672, #1673, #1758)** — `traceary session refine <session-id>` は `--summary` と `--covers-to`、任意の keywords/author、text または JSON output で agent-authored L2 summary を保存します。`covers-from` は session の最古の event から導出され、同じ coverage は `unchanged`、coverage の進展は earlier lower bound を保持した新しい generation で supersede します。Traceary は summary text を合成しません。compact hook は summary を捏造せず、実際に提供された host digest を使います。
- **起床時の finished session summary 注入 (#1684, #1714, #1785)** — 同じ workspace の eligible な summary を recalled plain text として client/session あたり最大 1 回、byte budget の範囲で新しい順に全文単位で注入します。入るものがなければ hook は何も書きません。Claude、Codex、Gemini、Grok は `SessionStart`、Kimi は初回 `UserPromptSubmit`、Antigravity は検証済みの `PreInvocation` output contract を使います。marker は spool replay と compact/resume の再発火をまたいで保持され、注入の失敗で hook 自体が失敗することはありません。
- **`traceary store payload-backfill` が既存履歴に codec を適用します (#1685, #1742)** — 後述の圧縮された canonical write が対象とするのは新規行だけです。このコマンドは既存の `events.body` と `command_audits` の `command_text` / `input_text` / `output_text` を、同じ versioned zstd codec で書き換えます。処理は上限付きの resumable なバッチで、live store に対して writer を凍結せずに実行します。identity と zstd が混在したコーパスはどのバッチ境界でも valid な store なので、中断しても修復作業は発生しません。これは copy 上でコストを見積もるだけの `store payload-rehearsal` とは別物です。実行前にバックアップを取得してください。body を一度圧縮すると v0.33.1 以前はそれを壊れたデータとして読みます。手順は `docs/storage/payload-backfill.ja.md` にあります。

### Changed
- **session resume の read を上限付きにし、normalized timestamp で順序付け (#1608)** — resume 用の session query は要求された window だけを stream し、全履歴を hydrate せず supporting index を使います。session の順序は raw timestamp text ではなく `ts_norm(started_at)` を使うため、同じ時刻を表す異なる timestamp 表現にも一つの public order が適用されます (#1729)。
- **search projection の lifecycle と tiered search を明示的な上限付き workflow に (#1611, #1612, #1622, #1634, #1639, #1679, #1680, #1733, #1735, #1740, #1793, #1794, #1798, #1801, #1810)** — store open は一度に上限付きの generation 作業を進め、`traceary store search-projection start|resume|status|abort` は durable checkpoint、過去 source inventory、session-tier readiness、exclusion、failure class、capacity evidence を公開します。`traceary search` は literal decode を authority とし、cutover 後の tail を統合し、event match と session summary/keyword match を別々に公開します。complete な generation では fingerprint pre-filter と session tier が使えます。incomplete、failed、drifted な generation は検証されていない session result を返す代わりに session result を返さず、そのことを MCP tool の `session_projection_not_ready` で伝えます。該当時は `traceary search` も stderr 通知で伝えます。candidate budget を使い切った場合は partial page ではなく `index_incomplete` を報告します。`--index-family-bytes` は recent tier の eviction に使う物理 index-family の target であり、index bytes や rebuild peak の上限ではありません。generation 単位の budget を超える行は無限に再試行せず除外して報告します。退役した migration-032 index は fallback になりません。
- **`store gc` が破棄前に未被覆の履歴を consolidation (#1674, #1676, #1683, #1719)** — refinement が被覆していない event range を上限付きの degraded mechanical summary に折り畳みます。1 回の pass の中では discard が**先**に、実行開始時点で既に存在した coverage に対して走り、consolidation はそのあとに続きます。つまりある run が折り畳んだものは次の run が破棄する対象になり、`--dry-run` は apply が実際に対象とする store をそのまま見ます。対応する stop hook を持つ host では、未精製の素材が設定した pressure threshold を超えたときに、作業の動機と変更内容を残し、経緯は任意とする consolidation を依頼します。gc 経路は fallback として残ります。discovery は range の件数で上限が設けられ、candidate 間の processing loop は wall-time budget で上限が設けられますが、いずれも処理中の candidate 一件を中断はしません。進捗は複数回の実行で引き継ぎます。読めない range 一つは件数に含めて skip し、全体を abort しません。残りの range を表示し、未要約の body を無言で破棄しません。
- **hook transcript の記録と重複修復を再開可能に (#1681, #1682)** — transcript recording は turn/content identity ごとに冪等になり、`traceary store dedupe content-events` は過去の hook prompt/transcript duplicate を中断可能な batch で監査または quarantine します。既定は dry run です。`--apply` は event を restore 可能な archive へ移し、`--restore` はそこから event を再作成します。archive が保持するのは event の論理的なフィールドであって全カラムではないため、codec や availability の metadata は復元ではなく再構成されます。`--purge` は rollback window を終了してから body を回収可能にします。candidate は hook の `prompt`/`transcript` event に限定され、command 実行はこれらの event を生成しません。ただしこれは candidate query の性質であって、audit テーブル側の不変条件ではありません。`--apply` は event 行を削除するため、その event を参照する `command_audits` 行があれば archive されないまま cascade delete され、`--restore` でも復元できません（#1862）。
- **retention accounting が encoded extent を報告 (#1759)** — codec metadata がある payload の retention plan は encoded payload extent から stored-byte total を計算し、legacy identity length は fallback としてだけ使います。表示される reclaimable amount は decoded plaintext bytes ではなく物理 stored bytes です。selection と restore の保証は変わりません。
- **payload backfill が長時間実行でも durable な terminal state に到達 (#1789)** — `traceary store payload-backfill` は bounded window 内の各 checkpoint transition を、完了を含めて記録します。そのため一回の invocation が transition window を超えても、実行中のままに見える状態を残しません。引き続き再開可能で、上記の compatibility warning の意味は変わりません。
...
- **レガシーの全文検索インデックスを退役 (#1718)** — migration-032 系列（`event_search_documents`、`event_search_fts`、`event_search_backfill_state`）は、bounded tiered projection が置き換えて以降どこからも読まれていませんが、書き込みのたびに維持され、ストア内に残り続けていました。長く使ったストアではファイル内で最大のオブジェクトになります（メンテナの環境では 24.7 GiB のうち 16.15 GiB、約 65%）。migration `000052` が初回起動時に維持を止めます。削除するのは writer trigger 8 個、`event_search_projection` view、`search_maintenance_control` だけで、いずれも定数コストです。migration は store を開くたびに無条件で適用されるため、数 GiB の `DROP TABLE` をそこに置くと、アップグレード後の最初の 1 回を拒否できないまま待たされるからです。テーブル本体の削除は明示的な別ステップにしました。`traceary store search-retire` が 3 テーブルを 1 トランザクションで削除します。冪等で（`already_removed`、exit 0）、数 GiB のストアでは数分かかることがありますが、中断してもロールバックされ何も変わりません。`DROP` はページを SQLite の free list に戻すだけなので、ファイルサイズを回収するには続けて `traceary store compact plan` / `apply` を実行してください。順序は固定です。`store compact plan` は当該系列が残っている間は失敗します（先に compact すると死んだ index を新しいファイルに焼き込むため）。残っている間は `traceary doctor` が保持バイト数と削除コマンドを示して警告します。**ロールフォワードのみです。** `store search-restore` はなく、バージョンを戻しても index は再構築されません。削除したデータが必要な場合は退役前に取得したバックアップから復元してください。必要なら事前に取得しておいてください。なお、family を残すことは downgrade の保険になりません。むしろ残したほうが危険です。v0.33.1 は 3 テーブルがすべて存在するかどうかで index を使うかを判断するため、残っている間はアップグレード時点で更新の止まった index を信頼し、不完全な結果を無言で返します。削除後は不在を検知して literal scan にフォールバックし、結果は正しくなります（走査するには広すぎるクエリを拒否するだけです）。（どちらにせよ downgrade には別のリスクがあります。#1618 の項目を参照してください。）検索自体は変わりません（本文一致は正本の復号走査がすでに判定していました）。詳細は `docs/operations/search-retirement.ja.md` を参照してください。
- **未使用の literal fingerprint candidate index を削除 (#1808)** — migration `000059` が、どの reader も依存していない 118,489,088 バイトの index を `literal_search_fingerprints` テーブルから削除します。2 つの fingerprint subquery は index の有無にかかわらず primary key で解決するため、query plan も generation も変わりません。migration は 7.1 GB の store で 0.096 秒でした（index の削除はページを書き換えずに解放するため）。これは実測値であって上限ではありません。他の migration と同じく store open 時に走り、wall-time の上限は設定されていません。SQLite は解放されたページを store 内で再利用しますが、`traceary store compact` / `VACUUM` を実行するまで filesystem には返却されません。
- **MCP から session-tier search 結果を公開 (#1727)** — MCP の `search` tool が、要約またはキーワードが query に一致する session trail を専用の `sessions` field で返し、`session_matches` または `session_projection_not_ready` reason を報告するようになりました。この schema 変更は意図的に `search` だけに限定しており、`list_events` と `get_context` は変更しません。
...
- **未リリースのtiered literal previewを削除 (#1725)** — undocumentedなCLIの`--tiered-preview` / `--deep` / `--continuation` entry pointと、MCPの`tiered_preview` / `deep` fieldを削除しました。これらは2026-08-02に追加されましたが、release tag（最新はv0.33.1）には一度も含まれていないため、deprecation windowは適用せず、bounded tiered path が唯一の検索経路として残ります。
- **canonical payload の圧縮書込み (#1618)** — v0.34 の canonical event と command-audit 書込みは、payload が小さくなる場合に versioned zstd codec を使い、それ以外は identity の TEXT を保持します。この変更は既存行を書き換えません。downgrade 用 fallback が必要なら、v0.34 を初めて実行する前に `traceary store backup create ~/traceary-pre-v0.34.db` を実行してください。v0.34 が圧縮行を書いた後は v0.33.1 以前の binary が body を garbage として読み、v0.34 の機能でそれを防ぐことはできません。`traceary doctor` は healthy な圧縮 store を warning にせず、representation と reader boundary を報告します。
- **command output の二重保存を停止 (#1675)** — 新規の `command_executed` は合成した `events.body`（`command` + `INPUT` + `OUTPUT`）を保存しません。アップグレード前に書かれた履歴の body は引き続き保持されます。新規書き込みで保持される実行記録は `command_audits` です。検索は body と独立に `command_text` / `input_text` / `output_text` を索引し続けます。list・doctor sensitive-path・replay failure hotspot は同一クエリで join した audit 列を読みます（行ごとの audit 再取得はしません）。body が空のとき、表示面は audit の command line を示します。履歴の reclaim は #1853 で別途扱います。
- **bounded search projection が pre-filter と session tier を提供します (#1717)** — projection 世代が complete かつ有効なとき、`traceary search` は fingerprint pre-filter で候補の復号を省略でき、session tier を別グループ `SESSIONS` として表示します。本文一致は正本テーブルを新しい順に走査して判定します。complete な世代がないストアでも検索はでき、本文一致の結果は正しいままです。tiered 経路は索引を参照するのではなく、候補を復号して 1 件ずつ判定するためです。ただし `SESSIONS` グループは世代が complete になるまで空であり、`traceary search` は stderr で session tier を参照しなかったことを伝え、`traceary store search-projection status` を案内します。state ごとに必要な操作は `docs/search-projection-rebuild.ja.md` にまとめています。MCP tool の `session_projection_not_ready` も従来どおり報告します（#1844）。失われるのは復号を省く能力で、大きなストアに対する広いクエリは candidate budget を使い切り、不完全なページを返す代わりに `index_incomplete` を報告します。したがって大きなストアで広い検索を使い続けるには再構築が必要です。v0.34 が退役させる migration-032 索引へのフォールバックはありません（#1718 の項目を参照）。3 文字未満のクエリはどの trigram 索引にも一致しないため、従来どおり bounded decoded scan で解決します。complete な世代はスナップショットなので、その後に記録された event は正本テーブルから読んで同じ結果に統合し、再構築の合間に検索結果が古くなることはありません。候補は source sequence 全体を公開順に走査し、保持するのは `offset+limit` 件だけなので、bounded window より多く一致しただけでクエリが利用不能になることはありません。
- **`traceary search --kind` が SESSIONS グループを空にしたことを伝えるようになりました (#1728)** — セッション要約とキーワードは kind 別の内訳を持たないため、`--kind` 指定時のセッションティアは絞り込まない結果を返すのではなく fail closed します。それを無言で行わなくなりました。`--kind` が指定されているときだけ、kind filter を外して同じセッション検索をもう一度実行し、一致があれば理由を stderr に報告します。報告するのは件数ではなく有無です。正確な件数を出すには、セッションティアから除外される event ページも含めて kind なしの検索全体を再現する必要があり、しかもその数字は利用者の次の操作を変えないためです。追加のクエリが走るのは `--kind` 指定時のみです。セッション行に対する filter の意味（`--from` / `--to` はセッションの開始時刻、`--failures` はセッション内のいずれかの失敗コマンド）は `docs/cli/README.ja.md` に明記しました。挙動自体は変更していません。
...
### Fixed
- **`store compact` が自身の手順書どおりの操作で deadlock しなくなりました (#1845)** — `plan` のあとに `store search-projection status` を実行すると 0 バイトの `-wal` と 32 KB の `-shm` が残り、以降の compaction コマンドはその存在だけを理由に、何度やっても同じ失敗を返し続けていました。抜け出す手段は無関係な書き込みコマンドを一度実行することだけで、それはどのメッセージにもドキュメントにも書かれていませんでした。`plan` は `apply` / `resume` と同様に preflight 全体で store の exclusive lease を保持するようになり、保持している間に限って stale な WAL 一式を拒否ではなく削除します。拒否する対象は変わりません。`-journal`、regular file でない sidecar、そして長さが非ゼロの `-wal`（commit 済み frame を保持しうるのはこれです）。`-shm` のサイズは判定に使いません。[SQLite WAL format](https://www.sqlite.org/walformat.html) のとおり database content を持たず fsync もされないためです。この recovery は「呼び出し側が lease を保持している」ことを明示的に宣言した場合にだけ有効なので、preflight 全体では lease を保持しない prepared-upgrade path は従来どおり拒否します。拒否メッセージは sidecar の種別とサイズ、および対処方法を示すようになりました。
- **projection 再構築が構築中もソーステキスト上限を維持するようになりました (#1796)** — eviction が source 走査の完了を待たず、走査と交互に実行されるようになりました。これにより導出された上限が、事後合計ではなく構築中の各時点で効きます。各バッチは永続化された `recent_source_bytes` カウンタを上限と比較し、追加投入の前にその世代の最古のドキュメントから evict するため、プロセス再起動をまたいでも上限が維持されます。従来は最後のソース行を走査し終えるまで投入分をすべて保持していました。408,893 イベントの store での実測では、`recent_source_bytes` は走査中ずっと 690,432,000 の上限に対し 690,430,718 に張り付いていました。マイグレーション `000058` はこのカウンタを追加し、旧バイナリが再構築途中で残した世代を再開始します。**上限されるのは保持ソーステキストであり、インデックスバイトではありません** — 後述の再構築ピークの制限を参照してください。予算が何を上限し何を上限しないかは `docs/search-projection-rebuild.ja.md` に明記しています。

### 既知の制限

v0.34 が約束するはずだったもののうち 2 つは約束できておらず、さらに構築しているものの 1 つはどこからも読まれていません。前者 2 つはリリースゲート（[#1620](https://github.com/duck8823/traceary/issues/1620)）で実測し、測定値のほうを調整するのではなく主張を取り下げました。3 つ目はリリースを切る作業中に判明したものです。黙って落とした主張は、まだ主張しているのと同じに読まれます。したがって 3 件ともここに明記します。この v0.34.0 エントリのゲート実測値は、以下のものも、上の Fixed 節に引用したものも、すべて `modernc.org/sqlite v1.55.0` で計測したものです。v0.34.0 が同梱するのは上流の journal rollback における corruption 修正を取り込んだ v1.56.0 なので、これらは出荷バイナリの正確な性質ではなく「その engine で計測した値」として報告します。

- **store は bounded ではありません。増加が遅くなっただけです (#1797)** — Wave 3 の施策でリファレンスコーパスは 24.71 GiB から 2.96 GiB になりましたが、レコードごとに増える分の大半は破棄できません。`select_discardable_event_bodies.sql` は `WHERE e.kind = 'transcript'` で始まり、`command_audits` は retention path から到達できません。30 日ウィンドウでの実測（300,282 イベント、非圧縮）では、破棄不能分がイベントあたり 2.67 KiB（うち `command_audits` テキスト 2.12 KiB、`prompt` body 0.55 KiB）、gc がいずれ破棄できる transcript body はイベントあたり 0.35 KiB です。つまりレコードが積む量のおよそ 80% は決して破棄できません。これは利用のしかたではなく設計の性質です。月あたりの数値ではなくレコードあたりで書いているのは、月あたりが「レコードあたり x 月あたりレコード数」であり、このソフトウェアに属するのは前者だけだからです。コマンド出力や入力プロンプトまで破棄対象を広げる案は検討のうえ却下しました。オペレータが生成した内容を不可逆に削除して数値を買う手段だからです。
- **`--index-family-bytes` は定常状態の目標値であり上限ではありません。再構築ピークを上限するものはありません (#1796, #1753)** — #1796 が上限するのは recent ティアが保持する**ソーステキスト**で、これは構築中のどの時点でも維持されます。インデックスバイトは上限しません。408,893 イベントの store、上限 1.43 GiB での実測では、`recent_source_bytes` が 690,432,000 の上限に張り付いたまま、インデックスファミリは 3.99 GB から 10.32 GB に増え、その後 cleanup バッチ 5 回で 14.31 GB 以上に達し、そこでボリュームの空き容量が尽きて `database or disk is full` で停止しました。したがって 14.31 GB はピークではなくピークの下限です。**空き容量を設定予算から見積もらないでください。** 再構築が容量不足で失敗しうることを前提にしてください。projection は store を開くたびに自動で catch-up します。容量が厳しい場合は `traceary store search-projection abort` で世代を永続的に park できます。自動 catch-up は再開せず、`resume` も failed 状態の世代を受け付けないため再開できません。先へ進む手段は `traceary store search-projection start` だけで、これは世代を**置き換えて**最初から再構築します。abort した実行の途中までの進捗は引き継がれません。`index_family_within_budget = -1` は**不明**を意味し、「予算以下」を意味しません。数 GB の store では判定に必要な測定が期限内に終わらないため、構築中ずっとこの値になります。詳細は `docs/search-projection-rebuild.ja.md` を参照してください。
- **recent 全文ティアは現在 write-only です (#1842)** — 構築・上限管理・evict・回収・測定は行われますが、どこからも読まれていません。`8cae0ab0` が FTS による候補選択を decode walk へ置き換えた際に最後の reader が失われ、v0.34 を切る段階までどの surface もそれを検知していませんでした。したがって `--index-family-bytes` が買うのは検索到達範囲ではなく保持量であり、大きな store では通常このティアがファイル内で最大のオブジェクトです（#1620 の実測では 17.4 GB のファイル中 9.0 GB）。検索結果には影響しません。このティアを空にしても結果は変わりません。削除するか検索から読ませるかは #1842 で決めます。
...
- PR #1587（`129fa58b`）とPR #1589（`bcda8f4a`）で完了したlocal test隔離を含むrelease QAは、[v0.33.0 release QA](docs/release/v0.33.0-qa.ja.md)に記録しています。
- install済みhost packageは現在のTraceary versionでrefreshしてください。`traceary doctor`はstale packageとrefresh済みpackageを区別します。
...
### Added
- **Kimi Code を first-class サポート (#1393)** — Kimi Code CLI（0.27.0 で live 検証）を Traceary のホストとしてネイティブ対応。`integrations/kimi-plugin/` パッケージとして提供し、1 つの `kimi.plugin.json` manifest に 10 個の lifecycle hook rule、Traceary MCP server、3 つの共有 memory/session skill を宣言。`scripts/install-kimi-plugin.sh` でインストールし（公式の local install を再現: generation dir へのステージ、atomic な symlink 切替、`enabled` 状態を保持した `installed.json` 更新）、`traceary doctor --client kimi` で確認できます。
...
### Fixed
- **Claude 版 session-history skill を共通文言に統一 (#1412)** — 他の 5 ホストが 1 つの文言に揃う間に Claude 版が drift していました。共有 skill parity check の下、全 6 ホストのパッケージが byte 一致になりました。

### Documentation
- **README の host テーブルに欠けていた Grok 行を追加 (#1409)** — 公開済みの host-coverage matrix と一致する内容で補完。
- **host coverage matrix の status 意味を明文化 (#1410)** — `wired` / `available` / `unsupported` を文書化（ホストの能力ではなく配線状態）、host-contract の語彙との関係も併記。
- **Kimi install script の swap 経路テスト (#1411)** — direct-copy からの移行、generation flip + prune、一時 link のクリーンアップを sandbox 化した振る舞いテストで固定。
...
### Added
- **Archive-before-GC dogfood 決定ログ (#1373)** — multi-GB store の copy 演習（dry-run/apply/verify）、automatic mode は既定 disabled、#1386 フォローアップの扱いを記録。
- **手動 archive-before-GC (#1371)** — `traceary store archive create|verify|restore` が GC 適格な cold 行を版付き package（gzip tar + manifest digest、任意で env 経由 passphrase）へ export し、完全性検証後に限り `--delete-after-verify` で厳密 identity 削除できます。restore は PK 冪等です。
- **Grok marketplace 公開パッケージ (#1301)** — marketplace catalog テンプレート + generator、clean-home install/update/uninstall smoke、二言語の投稿/導入 docs。ローカル install は決定的 fallback のまま。
- **opt-in automatic archive-then-gc (#1372)** — `config.json` の `retention.mode=archive_then_gc`（既定 `disabled`）が session start/end で opportunistic に archive+delete-after-verify を実行。DB 単位 lease、interval marker、passphrase は env 名のみ、doctor `archive-retention` で状態表示（秘密は出さない）。

### Fixed
- **opportunistic GC 下の stale-active-sessions backlog (#1363)** — session GC が hook soft deadline と競合しないよう切り離し（30s timeout）、session end でも実行、interval を 1h に短縮、`doctor --fix` で `session gc` を適用可能に。
- **バイナリ upgrade 後の host plugin version drift (#1361)** — ホスト別 refresh ヒントの明確化、post-upgrade チェックリスト docs、Grok install path 検出、Antigravity dual-path で健全 path があるとき不完全 twin を soft-skip。
...
- **rejected メモリも store gc の keep-days 対象に (#1369)** — `store gc --target memories`（および `all`）は `status=rejected` かつ `--keep-days` より古い行を `expired`/`superseded` と同様に物理削除します。accepted / candidate は対象外。dry-run の件数表示は従来どおりです。

### Notes
- v0.28.0 に破壊的 SQLite migration はなく、新しい MCP tool もありません。
- automatic archive-then-gc は **opt-in**（`retention.mode` 既定 `disabled`）。
- Grok public marketplace 掲載は `docs/integrations/grok-marketplace-submission.ja.md` に従い release tag の SHA を pin して xai-org/plugin-marketplace へ PR する運用です（ローカル install は継続サポート）。
...
### Fixed
- **退役した `integration` command stub の削除 (#1266)** — v0.20.x 以降に削除予定だった残骸 `traceary integration` subtree を取り除きました。unknown-command 挙動をテストと integrations verifier で固定します。

### Changed
- **doctor / hook_runtime のファイル分割 (#1259)** — 巨大化した `doctor.go` / `hook_runtime.go` を既存責務に沿って分割し、意図的な挙動変更なしでホスト作業の変更増幅を抑えます。
- **packaged hook script の single-source 化 (#1260)** — 互換ラッパーの正本は `scripts/hooks/*.sh`（prompt / compact / Grok を含む）です。package membership を明示し、`integrations sync-hooks` でコピーを再生成、`integrations verify` がドリフトで失敗します。
- **host coverage matrix の正本化 (#1261)** — lifecycle × host マトリクスの機械可読正本は `application/hostcoverage/matrix.json` です。日英 docs 表を生成・検証し、doctor の host-capability メッセージと event-coverage enrichment 期待値も同じ embedded matrix を読みます。

### Documentation
- **Grok 0.2.101 再 probe (#1299, #1300)** — live 再 probe でも standalone な `PostToolUseFailure` / `PermissionDenied` / `SessionEnd` は未発火で、subagent spawn は `spawn_subagent` tool のみ（`SubagentStart` / `SubagentStop` の parent/child hook 契約なし）。coverage は `docs/hooks/host-contract.json` と unavailable 設計メモに version 付きで残します。

### Notes
- v0.25.0 に破壊的な SQLite migration はなく、新しい MCP tool もありません。
- **明示的に延期（#1312 に記録）:** #1301 の public Grok marketplace 公開。ローカルソース導入は引き続き `scripts/install-grok-plugin.sh` + 対応タグでの `traceary doctor --client grok` をサポートします。
- memory inbox decay/restore (#1264) と archive-before-GC (#1309) はこのリリース対象外です（後続マイルストーンで追跡）。
...
- **明示的に延期（#1310 に記録）:** #1264 memory inbox decay/restore と #1309 archive-before-GC は High-risk の multi-PR トラックのままです（後続マイルストーン。v0.25.0 では未出荷）。archive/GC の自動実行は導入時も fail-closed / opt-in です。このタグでそれらの機能を出荷したとは扱わないでください。
...
- **Grok doctor check (#1278)** — `traceary doctor --client grok` が CLI、plugin の有効状態とバージョン一致、project hook の trust、導入済み hook の厳密な契約、MCP/skill の内容、直近 event coverage を確認します。Grok inspect から得た hook target path や transcript 本文は表示しません。
...
### Added
- **Antigravity plugin の機能同等化 (#1249)** — 配布する Antigravity plugin に hooks だけでなく Traceary MCP server と memory skills を同梱しました。doctor は MCP 登録と配布版・インストール版のバージョン一致を検証し、古い plugin が残る場合は再インストール手順を示します。
- **現行 Antigravity の capture coverage (#1250)** — Antigravity hooks が現行 payload の prompt と transcript を関連付け、turn 全体の coverage を記録するようになりました。不完全な turn は無効化しつつ、旧形式の transcript data は保持します。doctor は transcript 本文を露出せず、件数を制限した metadata から coverage を報告します。
- **Codex compact と subagent の境界 (#1251)** — Codex plugin が `PreCompact` / `PostCompact` の境界 marker を捕捉し、subagent の stop event を親 session と関連付けます。対応する境界 semantics を docs と regression test で固定しました。
- **Codex plugin hook の trust 診断 (#1252)** — doctor は現行 Codex plugin hook 一式が完全に信頼されている場合だけ healthy と報告します。trust 状態が不完全または確認不能な場合は安全側に倒し、完全な信頼を確認するまで手動 fallback hook を保持します。
- **activity-aware な session 自動 GC (#1253)** — hook ingestion が開始時刻ではなく最新 activity に基づく、rate limit 付き stale-session cleanup を実行します。process 間 activity lease は atomic に公開され、自動的に期限切れとなり、同時実行中の session を cleanup race から保護します。
- **hook write path の durable queue (#1269, #1270)** — host の timeout kill で中断された hook event を durable spool に保持し、turn ごとの memory extraction を durable な非同期 queue で実行します。同期 capture path から timeout と extraction latency の影響を外しつつ、待機中の処理を失いません。
- **Codex hook 重複登録の修復 (#1271)** — doctor が Codex hook の重複登録を検出し、無関係な hook 設定を維持したまま lossless かつ atomic に修復できるようになりました。

### Fixed
- **Claude SessionEnd cancellation の reconciliation (#1254)** — 解決済みの Claude hook cancellation marker を照合し、doctor が現在対応すべき SessionEnd failure だけを表示するようになりました。

### Changed
- **Gemini CLI の maintenance mode (#1255)** — Gemini extension は既存 install 向けに維持しますが、Google による Antigravity CLI への移行後は maintenance-only とし、新規 install には Antigravity plugin を案内するよう docs を更新しました。
...
### Added
- **履歴上の hook content の restore 可能な dedupe (#1227)** — `traceary store dedupe content-events` は履歴上の hook 由来 `prompt`/`transcript` の duplicate 行を監査し、`--apply` を付けると hard delete せず、event を復元可能な archive へ隔離します。既定は何も変更しない dry-run です。`--restore <run-id>` は apply が archive した event を再作成し（all-or-nothing で、既存 event id の上書きを拒否）、`--client codex|all` で agent スコープを指定し、`--strict` は時間差に関係なく完全一致 duplicate group をすべて報告し、`--json` はすべての mode で利用できます。duplicate の identity は `content-event-reliability` 診断と一致します（`kind, client, agent, session_id, workspace, source_hook, trim 済み body`）。write-side guard は trimming なしの exact body を使うため異なります。既定ではほぼ同時（診断と同様に連続レコードをペアで cluster する 10s の近接 window）の group のみが対象で、group ごとに残す canonical 行は Go 側で parse した最も早い RFC3339Nano の `created_at`（同値は event id で tie-break）です。malformed timestamp の group はスキップして報告し、command audit は candidate として選ばれません（この表現が隠していた cascade の危険は #1862 を参照してください）。apply は transaction 安全かつ冪等で、隔離した行は `events` の外へ移動されるため通常の `list`、`sessions --snapshot`、`doctor`、`context`、MCP の read surface から消えます。additive な schema migration `000019` が `event_content_dedupe_archive` テーブルを追加し、既存の `events` 行を移動・削除・書き換えしません。通常の upgrade/migration は自動クリーンアップを行いません。`content-event-reliability` の doctor チェックは新しい dry-run コマンドを案内し、現在の write path が新規 duplicate をすでに抑止していることを明記します。設計メモと rollback 手順は `docs/storage/README.ja.md` を参照してください。

### Documentation
- **Antigravity headless `agy --print` の capture level (#1225)** — headless な `agy --print` 実行では session start (`PreInvocation`) と `run_command` audit (`PreToolUse` + `PostToolUse`) のみが記録されることを明文化しました。print mode は `Stop` を発行しないため、最終 transcript / turn 境界は host が `transcriptPath` 付き `Stop` を発行する interactive 実行でのみ記録されます。これは想定どおりの capture level であり install 失敗ではありません — dogfooding (Traceary 0.21.2、`agy` 1.0.10) で確認済みです。英語/日本語の Antigravity 統合ドキュメントと host-coverage マトリクスにこれを正確に記載し、read 側の例を `traceary list --agent antigravity` に変更しました（hook event は `client=hook`, `agent=antigravity` で記録されるため `--client antigravity` では 0 件になります）。`agy` の起動や実 transcript の読み取りを行わずに print mode の capture level を固定する回帰テストを追加しました。

## [v0.21.2] - 2026-06-20

### Added
- **stale な Gemini インポート由来の Antigravity CLI plugin の検出 (#1220)** — `traceary doctor --client antigravity` に `antigravity-cli-plugin` check を追加しました。`~/.gemini/antigravity-cli/plugins/traceary` 配下の Antigravity CLI plugin 設定ファイル（`plugin.json` / `hooks.json` / `hooks/hooks.json`）のみを読み取り、サポート対象の Antigravity hook runtime へ移行した後も残った legacy な Gemini 形式の plugin ファイルを remediation 手順付きで警告します。サポート対象のトップレベル `traceary` hook group では pass し、CLI plugin ディレクトリが存在しない場合は skip するため、`traceary hooks install --client antigravity` ユーザーが失敗扱いになりません。
- **docs** — 英語 / 日本語の Antigravity integration docs に、stale な Gemini インポート由来の CLI plugin の移行手順を記載しました。
...
### Added
- **Antigravity hook / plugin の正式サポート (#1213)** — Antigravity を diagnostics 専用から実際の Traceary hook client に昇格しました。`traceary hooks print/install --client antigravity` が Antigravity の hook-group 形式の `hooks.json`（workspace の `.agents/hooks.json` または user-level の `~/.gemini/config/hooks.json`）を生成・マージし、`traceary` 以外の hook group をすべて保持します。リポジトリには公式 Antigravity plugin スキーマに従う `plugin.json` と同じ `traceary` hook group を同梱した `integrations/antigravity-plugin/` を追加しました。
- **Antigravity イベントカバレッジ** — `PreToolUse` / `PostToolUse` を `stepIdx` で対応付けて `run_command` の audit を記録し、`PreInvocation` で `conversationId` をキーにした冪等なセッション開始 / リフレッシュを行い、`Stop` でセッションを閉じずに turn transcript と turn boundary を記録します。
- **Antigravity doctor check** — `traceary doctor --client antigravity` が `antigravity-capability`（CLI / app-bundle 検出。credential 読み取りや browser 自動操作は行わない）と `antigravity-config`（解決された `hooks.json` に `traceary` group が登録されているか、未登録時は `--upgrade` を提示）を報告します。
- **docs** — integration / hooks / lifecycle / release の docs で Antigravity をサポート対象 host として記述し、`PreToolUse` / `PostToolUse` の対応付け、`Stop` の transcript / turn boundary、公開 hook surface の制約を説明しました。
...
- **command audit payload の上限処理 (#1173)** — command audit の input/output payload を保存前に切り詰めるとき、head / tail の文脈を残し、CLI / MCP の write result に original byte metadata を出し、`audit.max_input_bytes` / `audit.max_output_bytes` の config 既定値で調整できるようにしました。
- **sessions snapshot の late-event 反映 (#1172)** — `traceary sessions --snapshot` / `--json` がセッション終了後に遅れて届いたイベントを正しく反映するようになりました。GC 後のアクティブセッション検索も確実に解決します。
- **hook の重複書き込み抑制 (#1167)** — hook pipeline がリトライ窓内の prompt / transcript 書き込みを重複除去し、hook 再発火による冗長な row を防ぎます。
- **memory 候補の hygiene (#1169)** — `traceary sessions --snapshot --json`（`reliability.memory.candidate_hygiene` フィールド）が hygiene カウントを報告し、抽出時に明らかな code / diff 断片をレビューキューから除外することでオペレーターが意味のある候補だけを確認できます。
- **Claude イベントカバレッジ診断 (#1174)** — `traceary doctor` が Claude のイベントカバレッジのギャップを検出し、hook のキャンセル漏れを専用の diagnostic として表示します。
- **Gemini カバレッジ警告 (#1171)** — `traceary doctor` が Gemini セッションデータで boundary-only または audit-only なカバレッジを検出した場合に警告を出し、計装の欠落を示します。
- **Gemini compact 計装 docs と doctor check (#1176)** — `traceary doctor` が Gemini の compact イベント計装を検証し、docs で実環境での compact カバレッジの確認方法を説明します。
- **Antigravity 機能検出 (#1195)** — `traceary doctor --client antigravity --json` が Antigravity のインストール状況を調査し、`not_installed`（アプリバンドルも CLI も見つからない）・`tool_unavailable`（アプリ/CLI は存在するが公開の headless/hook/package サーフェスが未確認）・`not_authenticated`（将来/予約済み：サポートされたサーフェスでインストールされているが認証または設定が未完了。認証情報を読み取るのではなくサポートされた CLI/contract チェックで検出）・`available`（将来）の 4 ステートのいずれかを報告します。ローカル開発環境では `/Applications/Antigravity.app`（バージョン 2.1.4）がインストールされており公開 CLI/hook contract は未確認のため、現在のステートは `tool_unavailable` です。Antigravity はデフォルトの doctor クライアントリスト（`["claude","codex","gemini"]`）に含まれません。`--client antigravity` を明示的に指定してください。

### Fixed
- **audit-reliability 診断の重複 (#1168)** — audit-reliability doctor check が同一候補グループに対して重複した diagnostic を出さなくなりました。
- **Claude hook diagnostic cleanup matching (#1174)** — cleanup matching を厳密化し、キャンセルされた Claude hook 呼び出しが無音でドロップされず正しくキャプチャされるようになりました。
- **SQLite のタイムスタンプ比較 (#1185)** — SQLite クエリのタイムスタンプ比較を正規化し、秒以下の精度フォーマットの差異による境界ミスマッチを解消しました。

### Changed
- **Codex Stop はターン境界 (#1170)** — `Codex Stop` をセッション終了ではなくターン境界イベントとして正しく説明するよう、docs と診断を修正しました。実際の Codex host の挙動に合わせた修正です。
- **Clean My Agent 互換性の設計ノート (#1177)** — Traceary と Clean My Agent プロトコルの互換性方針を示す設計ノートを追加しました。
- **Gemini CLI をレガシーに変更、Antigravity 移行状況ページを追加 (#1197)** — README・統合ガイド・Gemini extension docs で、Gemini CLI が Google AI エージェントホストのレガシー実装であり、Antigravity がアクティブな後継であることを明記しました。新設した [Antigravity 移行状況ページ](./docs/integrations/antigravity.ja.md) は v0.21.0 時点でのローカル調査結果（app パス・bundle ID・バージョン・URL スキーム・ユーザーデータディレクトリ）を記載し、公開 CLI / hook contract が未確認であることを示しています。Antigravity の capability detection は #1195 で実装済みです。#1196 の判断として、v0.21.0 では Antigravity の hook・package・生成メタデータ・リリースアセットを意図的に同梱しません。#1195 でサポート対象の公開 CLI / hook contract が存在しないことが確認されたためで、`doctor` は偽の package を同梱する代わりに Antigravity を `tool_unavailable` として報告します。Gemini CLI extension package は既存インストール向けに引き続き提供されます。

### Notes
- v0.21.0 には SQLite schema migration と新しい MCP tools の追加はありません。hook 診断・memory hygiene・audit 正確性・session snapshot 精度に focused した dogfood reliability リリースです。
- リリース前に実環境 Traceary workspace での dogfood 検証を実施しました (#1178)。
