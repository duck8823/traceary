...
### Changed
- **停止した search projection が越えられない checkpoint を明示 (#1804)** — 最小バッチサイズでも 1 行が projection の lock duration cap を超える場合、store の起動時に `search projection catch-up incomplete; retrying on next initialization` と記録していました。再試行自体は起きますが毎回同じ理由で失敗するため、このメッセージは実在しない一時的状態を伝えていました。その間、大きな store では広い検索が `index_incomplete` を返し続けます。今後は generation が checkpoint を越えられないこと、その checkpoint、および復旧手順（generation を abort し、より大きな lock duration で start する。`resume` は generation と異なる budget を拒否するため）を記録します。行のスキップも新たな状態の永続化も行いません。継続的な書き込み競合と実際に遅い行はここでは区別できず（driver の busy timeout 1 秒は lock cap 250 ミリ秒の 4 倍のため、ブロックされた書き込みも同じ deadline として現れ、checkpoint は作業と同一 transaction で進みます）、その根拠で行を除外すると、混雑した store で正常な event を index から静かに落とすことになります。lock 待ちと行の処理時間の分離は #1833 です。
