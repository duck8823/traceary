...
### Known limitations
- **The recent full-text tier is currently write-only (#1842)** — it is built, bounded, evicted, and measured, but read by nothing. Therefore `--index-family-bytes` buys retention rather than searchable reach, and on a large store this tier is typically the largest object in the file. #1842 resolves this.
