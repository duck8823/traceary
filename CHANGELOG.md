...
### Removed
- **`traceary store search-projection` (#2077)** — start/resume/status/abort/probe fail as unknown subcommands (non-zero, no `DEPRECATED` notice). Parked recovery is `doctor --fix` (start if needed, then bounded resume). A new generation or a lock-time resume of an in-flight rebuild is `store compact --projection-rebuild` (same budget flags, including `--index-family-bytes`). Abort is `store compact --projection-abort`. Status and budget stay on `doctor` (`search-projection-parked` / `search-projection-budget`). Large-store default doctor still does not open SQLite or walk dbstat.
