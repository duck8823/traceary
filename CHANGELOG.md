...
### Fixed
- **`sessions --snapshot` returns on a large store (#1863)** — lineage expansion queried a session's family once per member of that family. `LineageOf` resolves every member of a family to the same root and therefore the same lineage set, while `scanSessionSummary` hydrates every returned row's latest-event body and `expandSessionLineages` deduplicated only afterwards — so a family with k active members was queried and body-hydrated k times and k-1 of those results were discarded. The same-family invariant had been broken by both the ancestor climb's depth cap and its comma-collidable raw-ID path guard; removing the cap and hex-encoding path IDs establish it, so expansion can safely skip a session already collected by an earlier family. The returned lineage set remains subject to the descendant depth cap; its completeness limitation is tracked as #1867. Measured on a 32.45 GB store with 27,540 sessions, 485 of them active: `--limit 100` took 32.2 s before and 26.3 s after, `--limit 200` took 642.7 s before and 98.9 s after, and the default snapshot did not return within 30 minutes before and completed in 201 s after; `--limit 100` output was byte-identical. The cost that remains after this change is tracked as #1865.
