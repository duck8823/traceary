...
### Changed
- **The leftover compatibility surface is removed (#2322).** Offline migration 86 drops `event_metadata_projection.legacy_source_hook` (and its CHECK) after refusing non-NULL values, recreates both `event_metadata_projection_events_after_insert` and `event_metadata_projection_events_after_update` without the column, and raises `minimum_reader_version` to 42. Runtime capability probes whose subjects initialize already guarantees are gone (event-delivery `hook_delivery_attempts` / `session_workspace_observations` table skips, workspace identity catch-up, attestation codec). The `observation_count` write-path probe stays: 076 is data-dependent offline, so live open still writes the pre-collapse row until `doctor --fix`. Canonical event/audit codec probes stay on the 082 source-read path (the source still has `body_codec` until 082 applies). `session_workspace_aliases` stays: named readers still require it. The migration-032 search-index retire path stays with its doctor caller. `store compact` absorbing flags were already removed in #2326; remaining alias flags keep their own reasons. Missing `store_format_state` is fail-closed except for the pre-36 lineage (and empty bootstrap); a modern ledger or a foreign `events` file without format state is refused. `maximum_payload_format` was already dropped in #2323. DROP COLUMN moves pages to the freelist; it does not shrink the file without VACUUM or a candidate rewrite. Leaves parent #2315 open.
- **The memory graph is removed (#2327).** Domain model, usecase, datasource, GC target `memory_edges`, and the bundle export writer are gone. Offline migration 85 drops an empty `memory_edges` table on a #2328 candidate, raises `minimum_reader_version` to 41, and refuses a non-empty table with the row count plus the 0.48.2 `bundle export` procedure. New bundle exports omit the entry (manifest stays v2: subtraction, not a version bump). Legacy zero-row `memory_edges` sections skip after checksum and row-count validation; non-zero sections refuse atomically without importing anything. `bundle import --orphan-edges` is an unknown flag. Pin Traceary 0.48.2 to retrieve remaining edges; this binary has no converter. DROP moves pages to the freelist; it does not shrink the file without VACUUM or a candidate rewrite. Feeds #2321; leaves parent #2315 open.
- **Archive packages and the retention-plan surface are removed (#2326).** `store compact --archive` / `--archive-verify` / `--archive-restore`, the twenty archive/retention flags, `hook archive_auto`, `config.json` `retention.mode=archive_then_gc`, and the file-retention domain layer are gone (unknown flags/commands, not hidden no-ops). Offline migration 84 drops empty `archive_segments` on a #2328 candidate, raises `minimum_reader_version` to 40, and refuses a non-empty table with the row count plus the 0.48.2 restore/export procedure. Pin Traceary 0.48.2 to retrieve existing archive packages; this binary has no converter or compatibility reader. `bundle` and `store backup` are unchanged (backup writes no retention sidecar). DROP moves pages to the freelist; it does not shrink the file without VACUUM or a candidate rewrite. Leaves parent #2315 open.
