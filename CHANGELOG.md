...
### Changed
- **Report the range full-text search actually covers (#1838)** — `store search-projection status` reported more than thirty fields and none of them said which period `traceary search` can answer in full text. `recent_cutoff_norm` is the source-phase prefilter boundary derived at `Start`, not what survived eviction, and on real stores it is empty. Two additive fields, `recent_oldest_norm` / `recent_newest_norm`, now report the oldest and newest instants actually retained in the recent tier. They carry the read path's own condition — a complete generation, named as the active one — so they are omitted whenever search is not reading the projection (during a rebuild, after a drift, before the first generation completes) rather than describing rows search does not consult. This is the full-text range only; older events stay answerable through the session tier.
