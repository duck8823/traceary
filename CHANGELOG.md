...
### Added
- **Metadata-only capacity diagnostics (#1607, #1624)** — `traceary store capacity` emits versioned `traceary.capacity/v1` JSON containing aggregate page, object, payload-size-class, free-page, and WAL measurements; it never reads or emits stored values or identifiers, and omits object attribution when SQLite `dbstat` is unavailable. Growth guidance and warnings use the same metadata-only path; they do not claim that a configured retention or index budget is a filesystem cap. (The accompanying `store-benchmark` harness, which times the production query paths against an immutable copy, lives in the repository and is not part of the released binary.)
- **Resumable payload-compression rehearsal on a copied store (#1609, #1610, #1635, #1641, #1643, #1645, #1749)** — `traceary store payload-rehearsal preview|run|resume|scrub|rollback` checks live/copy identity, migration readiness, WAL and lock limits, writes only versioned zstd shadow rows on the copy, and returns aggregate metrics plus opaque hashes. Runs pause only at committed batch boundaries and resume with the identical configuration; migration, scrub, rollback, and WAL reservations are bounded, and text budgets are measured in UTF-8 bytes rather than characters. The live canonical event and audit rows are never activated or modified by this workflow, aliases are rejected, and the copied target must have a verified physical rollback artifact.
- **Safe, resumable physical store compaction (#1613, #1633, #1645)** — `traceary store compact plan|apply|resume|status|rollback` replaces in-place vacuuming with a preview-first run journal, bounded candidate verification, an atomic same-filesystem exchange, and a retained source-size rollback copy. It refuses unsafe identity, sidecar, legacy-index, compatibility, and free-space states rather than guessing; interruption is recovered with `resume`, and a reviewed run can be restored with `rollback`. This is physical reclamation, not automatic GC, and the rollback artifact remains after success until the operator accepts and removes it.
...
- **Drop the unused literal fingerprint candidate index (#1808)** — migration `000059` removes a 118,489,088-byte index from the `literal_search_fingerprints` table that no reader depends on: both fingerprint subqueries resolve through the primary key with or without it, so no query plan and no generation changes. The migration itself is bounded — measured at 0.096s on a 7.1 GB store, because dropping an index frees pages rather than rewriting them. SQLite reuses those pages inside the store; they are not returned to the filesystem until `traceary store compact` / `VACUUM`.
