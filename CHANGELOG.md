...
### Changed
- **Name the checkpoint a stalled search projection cannot pass (#1804)** — when one row exceeds the projection lock-duration cap at the minimum batch size, store open logged `search projection catch-up incomplete; retrying on next initialization`. The retry does happen, and it fails identically on every open, so the message described a transient condition that is not one — meanwhile broad searches on a large store report `index_incomplete` for as long as the checkpoint stays put. The log now says the generation cannot advance past its checkpoint, names that checkpoint, and states the recovery: abort the generation and start a new one with a larger lock duration, because `resume` refuses a budget that differs from the generation's. Nothing is skipped and no new state is persisted. Sustained write contention and a genuinely slow row are not distinguishable here — the driver's 1s busy timeout is four times the 250 ms lock cap, so a blocked write surfaces as the same deadline, and the checkpoint advances in the same transaction as the work — and excluding a row on that evidence would silently drop good events from the index on a busy store. Separating lock wait from row work is #1833.
