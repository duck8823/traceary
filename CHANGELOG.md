...
### Changed
- **Compressed canonical payload writes (#1618)** — v0.34 canonical event and command-audit writes use the versioned zstd codec when it reduces the payload, otherwise preserving identity TEXT. Existing rows are not rewritten by this change. Once compressed rows exist, v0.33.1 and earlier cannot read them; run `traceary store backup` before upgrading when a downgrade fallback is required. `traceary doctor` reports this store state.
