...
### Fixed
- **A projection rebuild no longer peaks far above its index-family budget (#1796)** — eviction is now interleaved with the source walk instead of waiting for it to finish, so the derived source-text ceiling bounds the peak rather than the post-hoc total. Each batch compares a persisted `recent_source_bytes` counter against the ceiling and evicts the generation's oldest documents before admitting more, so the bound survives a restart. Previously a rebuild held everything it had admitted until the last source row was walked: measured at 3.74 GiB against a 1.43 GiB ceiling with 64% of the walk still to go. Migration `000058` adds the counter and restarts any generation left mid-rebuild by an older binary. What the budget does and does not bound — the evictable recent tier versus the corpus-proportional session and fingerprint tiers, and the two generations a rebuild holds at once — is now stated in `docs/search-projection-rebuild.md`.
