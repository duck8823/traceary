...
### Fixed
- **`sessions --snapshot` returns on a large store (#1863)** — lineage expansion queried a session's family once per member of that family. `LineageOf` returns the whole family rooted at the topmost ancestor and `scanSessionSummary` hydrates every returned row's latest-event body, while `expandSessionLineages` deduplicated only afterwards — so a family with k active members was queried and body-hydrated k times and k-1 of those results were discarded. Expansion now skips a session already collected by an earlier family. That is output-preserving because a session that appears in another session's family shares its root and therefore its family. Measured on a 32.45 GB store with 27,540 sessions, 485 of them active: the default snapshot did not return within 30 minutes before and completes in 201 s after; at `--limit 200`, 642.7 s before and 98.9 s after, with byte-identical output at `--limit 100`. The remaining cost is linear in active sessions, about 0.25 s each (#1865).
