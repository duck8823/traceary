...
### Changed
- **The leftover compatibility surface is removed (#2322).** Offline migration 86 drops `event_metadata_projection.legacy_source_hook` (and its CHECK) after refusing non-NULL values, recreates both `event_metadata_projection_events_after_insert` and `event_metadata_projection_events_after_update` without the column, and raises `minimum_reader_version` to 42. Runtime capability probes whose subjects initialize already guarantees are gone (event delivery, workspace identity catch-up, attestation codec, canonical event/audit codec). `session_workspace_aliases` stays: named readers still require it. The migration-032 search-index retire path stays with its doctor caller. `store compact` absorbing flags were already removed in #2326; remaining alias flags keep their own reasons. Missing `store_format_state` is fail-closed except for the pre-36 lineage (and empty bootstrap). `maximum_payload_format` was already dropped in #2323. DROP COLUMN moves pages to the freelist; it does not shrink the file without VACUUM or a candidate rewrite. Leaves parent #2315 open.
