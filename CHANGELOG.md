...
### Changed
- **Disclose retention-plan discard counts (#1786)** — `store retention plan` now shows the number of candidate bodies it will discard and how many reclaim essentially nothing because their encoded extent is at or below the retention marker. Selection is deliberately unchanged: `--keep-days` is a retention policy rather than a space optimizer, recovery is mandatory for every candidate, and the v2 `reasons` field is a frozen `['age']` contract that cannot disclose a second reason without breaking plan application.
