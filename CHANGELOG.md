...
### Added
- **Metadata-only capacity diagnostics and a copied-store benchmark (#1607, #1624)** — `traceary store capacity` emits versioned `traceary.capacity/v1` JSON containing aggregate page, object, payload-size-class, free-page, and WAL measurements; it never reads or emits stored values or identifiers, and omits object attribution when SQLite `dbstat` is unavailable. The benchmark command runs against a consistent immutable copy, reports bounded p50/p95 timings and query plans for the production active/latest/handoff/search paths, and records timeout lower bounds instead of inventing timings. Growth guidance and warnings use the same metadata-only path; they do not claim that a configured retention or index budget is a filesystem cap.
