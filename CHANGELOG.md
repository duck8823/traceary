...
- **Compressed canonical payload writes (#1618)** — v0.34 canonical event and command-audit writes use the versioned zstd codec when it reduces the payload, otherwise preserving identity TEXT. Existing rows are not rewritten by this change. Take `traceary store backup` before first running v0.34 if you need a downgrade fallback: after v0.34 has written compressed rows, v0.33.1 and earlier read those bodies as garbage, and nothing in v0.34 can prevent that. `traceary doctor` reports the representation and reader boundary without warning on a healthy compressed store.
