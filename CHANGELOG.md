...
- **Repository tooling migrated to Go (#1118, #1119, #1120, #1121)** — maintainer-only repository verifiers moved from Python to a single `go run ./cmd/repo-tooling ...` entrypoint: `integrations verify`, `docs verify-i18n`, `release verify-changelog`, and `docs verify-landing` replace the four `scripts/verify_*.py` helpers, with CI, the Makefile, the release workflow, and CONTRIBUTING wired to the Go commands. Only `scripts/bump_version.py` remains Python.
