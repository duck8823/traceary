...
- **Stop storing command output twice (#1675)** — `command_executed` events no longer persist a composed `events.body` (`command` + `INPUT` + `OUTPUT`). The retained execution record is `command_audits` only. Migration `000048` (`data_dependent_offline`) clears historical `events.body` for `kind='command_executed'`. Search continues to index `command_text` / `input_text` / `output_text` independently of body. List, doctor sensitive-path, and replay failure hotspots read the joined audit columns in the same query (no per-row audit lookup). Display surfaces show the command line from the audit when the body is empty.
