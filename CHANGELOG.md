...
### Changed
- **Drop the unused literal fingerprint candidate index (#1808)** — migration `000059` removes the 118,489,088-byte redundant index from the `literal_search_fingerprints` table. The two fingerprint reader subqueries keep the same primary-key query plans, so no generation is invalidated. SQLite reuses the freed pages inside the store; they are not returned to the filesystem until `traceary store compact` / `VACUUM`.
