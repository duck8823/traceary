...
- **Bounded search projection is the authoritative read path (#1717)** — when a projection generation is complete and active, `traceary search` reads recent full-text hits from it and answers older history through session summaries and keywords, shown as a separate `SESSIONS` group. A store that has not completed a generation still searches, and still searches correctly: the tiered path decides every candidate by decoding it rather than by consulting an index. What it loses is the ability to skip decoding, so a broad query on a large store exhausts the candidate budget and reports `index_incomplete` rather than returning a partial page. Rebuilding is therefore how a large store keeps broad search available — there is no fallback to the migration-032 index, which v0.34 retires (see the entry below). Queries shorter than three characters match no trigram index and resolve through the bounded decoded scan, unchanged. Because a completed generation is a snapshot, events recorded after it are read from the canonical tables and merged into the same result, so search does not go stale between rebuilds; candidates are visited in public order across the whole source sequence and only `offset+limit` matches are retained, so a query does not become unavailable merely because more rows match than the bounded window holds.
