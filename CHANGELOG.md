...
### Fixed
- **`sessions --snapshot` returns on a large store (#1863)** — lineage expansion queried a session's family once per member of that family. `LineageOf` returns the whole family rooted at the topmost ancestor and `scanSessionSummary` hydrates every returned row's latest-event body, while `expandSessionLineages` deduplicated only afterwards — so a family with k active members was queried and body-hydrated k times and k-1 of those results were discarded. The ancestor climb's depth cap made `LineageOf` return different sets for different members of one family. Removing that cap establishes the same-family invariant — it never held before — so expansion can safely skip a session already collected by an earlier family. Measured on a 32.45 GB store with 27,540 sessions, 485 of them active: `--limit 100` took 32.2 s before and 26.3 s after, `--limit 200` took 642.7 s before and 98.9 s after, and the default snapshot did not return within 30 minutes before and completed in 201 s after; `--limit 100` output was byte-identical. The cost that remains after this change is tracked as #1865.
