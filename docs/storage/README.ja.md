...
- **定常状態**: complete な 1 世代分 — session summaries、command aggregates、session keywords、literal fingerprints、および `--index-family-bytes` 内の recent-document ティア。session keywords と literal fingerprints はコーパス比例で evict できません。family 予算が抑えるのは recent ティアだけです。
...
**概念モデル。** duplicate group は identity tuple `kind, client, agent, session_id, workspace, source_hook, TrimSpace(body)` です。これは `content-event-reliability` 診断が使う identity と同じで、どの行が grouping に**参加できるか**（eligibility）についても両者は一致します。両方とも retention で空にされた行を除外するため（後述）、診断が報告する duplicate 件数は本コマンドが実際に処理する件数と一致します。これは履歴クリーンアップ用の推定であり、実行時の delivery identity ではありません。書き込み時に再送を抑止するには、ホスト由来の安定した delivery ID と semantic fingerprint の一致が必要で、本文の一致だけでは同一性を証明しません。対象は `client='hook'` かつ `kind in (prompt, transcript)` の行のみで、**command audit は対象外** です。既定では、メンバーがほぼ同時（診断と同様に連続レコードをペアで cluster する 10s の近接 window 内）に書かれた group のみが対象になり、離れた意図的な再送は除外されます。`--strict` はこの window を外します。group ごとに残す **canonical** 行は、parse した `created_at` が最も早いもの（同値は event id が小さい方）です。`created_at` は Go 側で RFC3339Nano として parse します（`formatTimestamp` は可変幅の小数秒を出力するため、辞書順では並びません）。malformed な timestamp を含む group は **スキップして報告** し、変更しません。

ただし非対称は 1 点だけ意図的に残しています。retention の **ledger** 行（後述）は grouping には参加し続けるため診断の duplicate group 件数にも入りますが、本コマンドはこの行を絶対に **archive** しません。「grouping に参加できるか（eligibility）」と「削除できるか（archivability）」は別の問いで、診断が写しているのは eligibility だけです。
