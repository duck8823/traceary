...
**A run that folds does not discard.** A dry run consolidates without writing, so its candidate count can only see refinements that already existed. If an apply folded first and then discarded, it would remove bodies the preview never counted — the one loss `--dry-run` exists to make visible. So when a run produces any orphan refinement it reports `Cleanup skipped` and leaves the discard to the next run, whose preview now counts exactly what it will discard. Consolidation converges, so this costs one extra invocation, not one per run.
