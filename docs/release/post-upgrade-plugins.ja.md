...
6. synthetic マトリクスで read-side の保証を証明する。host 別 hook entrypoint（muse、grok、kimi。claude、codex、gemini、antigravity には synthetic seed がないため上記 live-capture gate の対象のまま）で使い捨て store を seed し、その store 上で search、session refine、memory store propose/search を検証します。store は 64 MiB 未満に制限し、終了時に消去します。
