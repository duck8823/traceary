...
4. **Dogfood remaining interactive surfaces when they change.** The operator cockpit and `TestCockpitDogfood` were removed in v0.35.0 (#1764). For releases that change `sessions`, `memory inbox review`, or `tail`, dogfood those surfaces directly (see the historical note in [`cockpit dogfood checklist`](../operations/cockpit-dogfood.md)).
