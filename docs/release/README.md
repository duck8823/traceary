...
2. **Bump manifests.** Run `make release/bump VERSION=X.Y.Z` — this updates `VERSION`, all integration plugin manifests, and runs both `scripts/verify_release_manifests.py` and `scripts/verify_integrations.py`.
3. **Verify locally.** `python3 scripts/verify_changelog_releases.py`, `python3 scripts/verify_release_manifests.py`, `go test ./...`, and `go tool golangci-lint run` must all pass. These Python entrypoints are still the current release-prep surface; the planned Go replacement is tracked in [`../operations/repo-tooling.md`](../operations/repo-tooling.md).
