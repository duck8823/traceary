...
4. Run the body-free plugin-version gate. Every unskipped host must have a `pass` plugin-version result. Antigravity may additionally report `skip` for an incomplete dual-path twin only when another copy passes:
...
5. Prove live capture with the sibling gate. **Plugin-version PASS is not capture** — the v0.47.0 dogfood passed `grok-plugin` while live Grok capture was zero. This gate re-runs the plugin-version identity gate first, then runs a headless one-shot probe per unskipped host into a fresh throwaway `TRACEARY_DB_PATH` and requires at least one `session_started` and one `prompt` in `traceary --db-path … list --json` output (plus `command_executed` for hosts named via `--require-command HOST`). It never requires or synthesizes `session_ended`, and it reads only JSON kind counts, never event bodies:
...
|---|---|---|---|---|
| Claude Code | Headless: `claude plugin marketplace update traceary-plugins`, then `claude plugin update traceary@traceary-plugins` (user scope) and the same command with `--scope local`. The bare name `claude plugin update traceary` fails with "Plugin not found". Interactive `/plugin update traceary` remains valid. Restart after update; resumed sessions may keep old snapshot hooks until fully restarted. If doctor reports multiple cached versions, restart every host session that could hold the old snapshot first, then remove the older cache directory — never the reverse. | Restart Claude Code (or start a new process) so the updated package is loaded. | `traceary doctor --client claude --json` → `claude-plugin-version` is `pass`. | `--skip claude='reason'` only when Claude Code/its Traceary package is intentionally not installed. |
| Codex | Run `codex plugin marketplace list` to identify the root. Git-clone: `git checkout` the tag matching `traceary -v` in that clone. Local-path (checkout is the marketplace root; manifest `.agents/plugins/marketplace.json`): `git -C <marketplace-root> checkout` the same tag. Then `codex plugin add traceary@traceary-marketplace`. Do not use `codex plugin marketplace upgrade` on a local-path marketplace (it reports "No configured Git marketplaces"). Interactive `/plugins` remains valid. | Start a new Codex session after the add (or `/plugins` refresh). | `traceary doctor --client codex --json` → `codex-plugin-version` is `pass`. Isolated throwaway-store capture of `session_started` / `prompt` is required for recording; plugin-version PASS is not capture. | `--skip codex='reason'` only when Codex/its Traceary package is intentionally not installed. |
...
| Grok Build | `./scripts/install-grok-plugin.sh` (installs the plugin **and** `traceary hooks install --client grok --global`, because Grok 1.0.5 executes `~/.grok/hooks` command files, not the plugin listing). Do not delete `~/.grok/hooks/traceary.json` while inspect lists the plugin as `hookType=file`. | Restart Grok Build or start a new session. | `traceary doctor --client grok --json` → `grok-plugin` is `pass` and `grok-hooks-user` is `pass` (or `grok-hooks` is `pass` only if inspect dispatches plugin-source **command** hooks). Isolated throwaway-store capture of `session_started` / `prompt` is required for recording; plugin-version PASS is not capture. | `--skip grok='reason'` only when Grok Build/its Traceary package is intentionally not installed. |
| Kimi Code | `./scripts/install-kimi-plugin.sh`. The installer stages a new generation and atomically flips the managed `traceary` symlink while preserving the install record. | Run `/plugins reload` **or start a new Kimi session**. | `traceary doctor --client kimi --json` → `kimi-plugin-version` and native `kimi-plugin` are healthy. Isolated throwaway-store capture of `session_started` / `prompt` is required for recording; plugin-version PASS is not capture, and `session_ended` is never required. | `--skip kimi='reason'` only when Kimi Code/its Traceary package is intentionally not installed. |

Prefer the `FixCommand` printed by doctor when it provides an exact non-interactive command. Do not invent host CLI flags.

## Restart before removing an older plugin cache

Doctor may report several versioned directories under a host plugin cache (Claude: `~/.claude/plugins/cache/...`). The host resolves that directory once at startup and keeps the absolute path. A resumed session (`--continue` / cmux) can still be running the older snapshot.

1. Fully restart every host session that could hold the old snapshot first (no resume).
2. Only then remove the older cache directory.

Removing the older directory while a session is still running unlinks the snapshotted plugin path. Recording stops, and doctor then reports the remaining cache as `pass`, so nothing in Traceary shows the gap. If you already removed an older snapshot from under a live session, restart the host to recover.
