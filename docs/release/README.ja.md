...
v0.23.0 以降、宣言された lifecycle hook と共有 memory/session skill を含む Grok Build のネイティブパッケージ（`integrations/grok-plugin/`）も提供します。Grok Build 1.0.5 は plugin 由来 hook を dispatch しないため、live capture は `~/.grok/hooks/traceary.json`（`traceary hooks install --client grok --global`。`scripts/install-grok-plugin.sh` も実行）を使います。詳細は [host カバレッジ](../hooks/host-coverage.ja.md) と [Grok Build plugin ガイド](../integrations/grok-plugin.ja.md) を参照してください。
...
- `integrations/claude-plugin/.claude-plugin/plugin.json`、`integrations/antigravity-plugin/plugin.json`、`integrations/gemini-extension/gemini-extension.json`、`integrations/grok-plugin/plugin.json`、`integrations/kimi-plugin/kimi.plugin.json`、`plugins/traceary/.codex-plugin/plugin.json` の `version` が root の `VERSION` と一致すること
...
3. **ローカル検証。** `go run ./cmd/repo-tooling release verify-changelog` / `python3 scripts/verify_release_manifests.py` / `go run ./cmd/repo-tooling docs verify-landing` / `go run ./cmd/repo-tooling docs verify-antigravity-status` / `go test ./...` / `go tool golangci-lint run` をすべて通します。残りの Python entrypoint は `cmd/repo-tooling` へ移行中で、移行順は [`../operations/repo-tooling.ja.md`](../operations/repo-tooling.ja.md) に整理しています。
4. **残る対話 surface を dogfood する。** operator cockpit と `TestCockpitDogfood` は v0.35.0 で削除されました (#1764)。`list --follow`、`memory inbox review`、残る対話 read を変更する release では、それらの surface を直接 dogfood してください（履歴メモは [`cockpit dogfood checklist`](../operations/cockpit-dogfood.ja.md)）。
...
10. **Homebrew formula PR を確認し、ローカル host package を更新する。** release workflow が `maintenance/homebrew-vX.Y.Z` PR を開いて auto-merge を有効化します。実際に merge されたか確認し、`brew update && brew upgrade traceary && traceary -v` で新バージョンを確かめます。その後、[更新後 plugin チェックリスト](./post-upgrade-plugins.ja.md) に従い、意図的に利用しない host ごとに明示的な `--skip HOST=REASON` を付けて `./scripts/verify-post-upgrade-plugin-refresh.sh` を実行します。この release QA gate は、導入済み package が core binary より古いと失敗します。最後に `./scripts/verify-post-upgrade-live-capture.sh` を実行します。identity を再確認したうえで、skip していない各 host が隔離された使い捨て store に `session_started` と `prompt` を実際に記録することを証明します。plugin-version の pass だけでは capture になりません。
