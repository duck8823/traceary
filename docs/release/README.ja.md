...
v0.23.0 以降、検証済み lifecycle hook、Traceary MCP server、共有 memory/session skill を含む Grok Build のネイティブパッケージ（`integrations/grok-plugin/`）も提供します。同じ release tag の checkout から `scripts/install-grok-plugin.sh` で導入し、`traceary doctor --client grok` で確認してください。詳細は [Grok Build plugin ガイド](../integrations/grok-plugin.ja.md) を参照してください。
...
4. **残る対話 surface を dogfood する。** operator cockpit と `TestCockpitDogfood` は v0.35.0 で削除されました (#1764)。`sessions`、`memory inbox review`、`tail` を変更する release では、それらの surface を直接 dogfood してください（履歴メモは [`cockpit dogfood checklist`](../operations/cockpit-dogfood.ja.md)）。
