...
v0.11.0 以降、hook 経由の session 終了 (`traceary hook session <client> end|stop`) は session 終了 record の commit 後に extract を best-effort で auto-fire します。エージェント側の明示要請なしに inbox が増え、extract のエラーは swallow されるため境界 record は決してブロックされません。
