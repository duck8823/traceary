...
scan は accepted memory に対して 3 種類の条件をチェックします: 現在の redaction ルールで mask されるべき内容 (`redaction_hit`)、`--expiry-days` 以上更新が無い stale row (`expiry_candidate`)、同一 scope + 同一 fact の衝突 (`duplicate`)。コマンドは read-only で、候補 id に対して `memory supersede` / `memory expire` / `memory reject` を実行することで transition を適用します。
