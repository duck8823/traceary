...
For Gemini, the read-only and apply PRs together record the same two artefacts that stand in for a flaky live launch:
...
- `scripts/smoke_test_gemini_activation.sh` materialises a temp project (`.git`, `GEMINI.md` seeded with a `## Gemini Added Memories` section, `.traceary/memories/gemini.md`). The script's structural section now exercises `--apply` end-to-end (first apply updates the host stub and creates the external file, a second apply converges to noop, `--status` reports `in_sync` afterwards) and asserts that the seeded `## Gemini Added Memories` section is preserved byte-for-byte. A CI-side regression in the apply path is therefore detected without launching Gemini. Set `TRACEARY_KEEP_SMOKE_TEMP=1` to retain the temp project for a manual `gemini` runtime probe; the live launch is gated behind `TRACEARY_ENABLE_GEMINI_RUNTIME_SMOKE=1` because Gemini CLI's hierarchical context loading and authentication state make an unattended runtime probe non-deterministic.

A maintainer-recorded `smoke_test_gemini_activation.sh` run with `TRACEARY_ENABLE_GEMINI_RUNTIME_SMOKE=1` is still required when an alternative gate is not available, or the ADR must be updated explaining why a different gate is acceptable.
