...
> **v0.35.0 (#1871):** MCP `get_report` は削除されました。この契約は `traceary report` のみに適用されます。
...
- CLI は、同じ期間・フィルター・ページサイズ・結果上限に対して安定した集計スキーマを返します。
...
3. CLI のスキーマと flag 契約の失敗テストを追加します。
4. 条件・範囲情報の値オブジェクト、report query adapter、usecase の順に実装します。
5. CLI 内の集計を置き換え、完全集計時の互換フィールドは変えません。

### リスクとロールバック

- 手続き化リスク: 期間・上限・割合判断を複数層に複製すること。対策として application の条件と snapshot を 1 つにします。
...
- ロールバック条件: CLI のスキーマ差、部分集計での割合表示、整数の巻き戻り、集計時の本文取得。変更は追加的で migration はありません。
