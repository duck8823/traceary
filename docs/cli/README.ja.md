...
取得するために `usage_observation_runs` と結合します。退役した `run_lineages`
table 由来のリポジトリ、チケット、プルリクエスト、バッチ、パケットバイト、
ツール出力バイトの帰属情報は報告しません。置き換えられた observation は除外され、
各 observation の最新かつ置き換えられていないスナップショットだけが集計されます。

run は `run_host` / `run_id` で重複排除し、engine ごとに wall-time のみを集計します。
パケットとツール出力のバイト数は報告しません。
