...
- 置き換え cockpit には `traceary sessions --snapshot` を使用してください。bare TTY の既定動作は v0.35.0 で `traceary --help` になります
