...
- `codex-memory-activation`: accepted durable memory が Codex native memory target で `missing` / `stale` / `in_sync` / `invalid` のどれかを確認し、更新が必要なら正確な `memory activate --dry-run --diff` / `--apply` command を表示します。
