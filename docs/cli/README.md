...
Manage operator-reviewed session/workspace aliases used by the current diagnostic projection. These flags absorb the former `store workspace-alias add|list|remove` leaves. Add requires `--session`, `--workspace`, and `--reviewed-by`; aliases never rewrite canonical provenance. `doctor --fix` does not invent aliases. Default doctor inspect is unchanged except an additive `workspace-aliases` check on the small-store path.
