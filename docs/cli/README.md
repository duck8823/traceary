...
When a bounded search projection generation is active (`search_projection_state.state = complete` with a non-empty `active_generation_id`), `search` reads that projection: recent full-text hits come from `search_projection_recent_fts`, and older history that only matches session summaries/keywords appears as a separate **SESSIONS** group. A SESSIONS row means "the trail is here, open it" — it is not a matching event line. Until a projection generation completes, the legacy migration-032 index remains the automatic silent fallback, so existing stores keep working without a rebuild.

A completed generation is a snapshot, so events recorded after it are read directly from `events` and merged into the same result — search never goes stale between rebuilds. If that tail has grown past the bounded candidate limit, `search` falls back to the legacy index rather than return a knowingly incomplete page; rebuilding the projection (`traceary store search-projection start`, then `resume`) restores the fast path.
