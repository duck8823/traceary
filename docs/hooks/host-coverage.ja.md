...
> **Grok native core runtime:** `traceary hooks install --client grok` は `.grok/hooks/traceary.json`（`--global` では `~/.grok/hooks/traceary.json`）へ書き込みます。core event は `client=hook`、`agent=grok` で保存します。`Stop` は turn 境界のままで、compact と subagent の補足は #1276 で扱います。
