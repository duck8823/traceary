...
| Traceary lifecycle event | Claude Code (`claude-plugin`) | Codex CLI 0.145.0 (`plugins/traceary`) | Gemini CLI (`gemini-extension`) | Antigravity (`antigravity-plugin`) | Kimi Code 0.27.0 (`kimi-plugin`) | Grok Build 1.0.5 | 確認方法 |
|---|---|---|---|---|---|---|---|
...
| `session_ended` | ● `SessionEnd` | ✕ host のセッション終了信号なし — Codex `Stop` は応答ごとの turn 境界でありセッション終了ではない (#1170)。終了は `traceary session end` または stale GC 経由 | ● `SessionEnd` | ✕ host のセッション終了信号なし — Antigravity `Stop` は execution 単位の境界でありセッション終了ではない (#1170)。終了は `traceary session end` または stale GC 経由 | ● `SessionEnd`（`reason` = exit） | ○ `SessionEnd` は文書化済み。1.0.5 の probe では teardown 時に user-level hook へ dispatch されたが、plugin 由来 hook（Traceary の経路）は dispatch されない | `traceary list events --kind session_ended --limit 5` |
