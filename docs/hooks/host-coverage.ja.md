...
|---|---|---|---|---|---|---|---|
| `session_started` | ● `SessionStart` | ● `SessionStart` | ● `SessionStart` | ● `PreInvocation`（`conversationId` を key にした冪等開始。Antigravity に `SessionStart` はない） | ● `SessionStart`（`source` = startup|resume。resume は同一 session_id で再発火し、冪等に記録） | ○ plugin は `SessionStart` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list --kind session_started --limit 5` |
| `prompt` | ● `UserPromptSubmit` | ● `UserPromptSubmit` | ● `BeforeAgent` — 配線済みだがアカウント eligibility に依存: `IneligibleTierError` で拒否されるアカウント (Gemini Code Assist for individuals は提供終了) では `-p` が `SessionStart` 直後に中断し prompt は記録されない (2026-08-21 dogfood probe)。ホスト単位の確認は `traceary doctor --client gemini` (`gemini-host-eligibility`) | ● `Stop`（直接の prompt field は無い。`transcriptPath` の最新 `USER_INPUT` / `USER_EXPLICIT` 行から復元） | ● `UserPromptSubmit`（`prompt` の content block 配列をテキスト化） | ○ plugin は `UserPromptSubmit` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list --kind prompt --limit 5` |
| `command_executed` | ● `PostToolUse` + `PostToolUseFailure` (Bash, `mcp__.*`, built-in tool matcher) | ● `PostToolUse` | ● `AfterTool` | ● `PreToolUse` + `PostToolUse`（`run_command`。command args は `conversationId + stepIdx` で 2 event をまたいで突き合わせ） | ● `PostToolUse`（`tool_output` 文字列）+ `PostToolUseFailure`（`error` object を平坦化。`PreToolUse` は検証のみで記録しない） | ○ plugin は `PreToolUse`/`PostToolUse` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list --kind command_executed --limit 5` |
| `transcript` | ● `Stop` | ● `Stop` (`last_assistant_message`) | ● `AfterAgent` | ● `Stop`（`transcriptPath`、best-effort な寛容 JSONL 走査） | ● `Stop`（best-effort: `session_index.jsonl` → session の `wire.jsonl` 最終ターンの `content.part` think/text block） | ○ plugin は `Stop` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list --kind transcript --limit 5` |
| `compact_summary` | ● `PostCompact` (+ `PreCompact` marker, `SessionStart matcher=compact` で resume) | ● `PreCompact` + `PostCompact` marker（`trigger` のみ。Codex は圧縮後サマリー本文を公開しない） | ● `PreCompress` (marker のみ — Gemini に post-compress 側 hook はない) | ✕ 文書化された compact hook なし | ● `PreCompact` + `PostCompact`（`trigger` の marker として記録。auto は live 観測済み、manual は未 probe。payload の token 数は保存しない） | ○ plugin は `PreCompact`/`PostCompact` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list --kind compact_summary --limit 5` |
| `session_ended` | ● `SessionEnd` | ✕ host のセッション終了信号なし — Codex `Stop` は応答ごとの turn 境界でありセッション終了ではない (#1170)。終了は `traceary session end` または stale GC 経由 | ● `SessionEnd` | ✕ host のセッション終了信号なし — Antigravity `Stop` は execution 単位の境界でありセッション終了ではない (#1170)。終了は `traceary session end` または stale GC 経由 | ○ `SessionEnd` は文書化済みで plugin も宣言しているが、0.38.0 の隔離 `-p` probe（2026-08-21）では `session_started`/`prompt`/`transcript` は記録されても SessionEnd は dispatch されなかった。最終 live 観測は 0.27.0 | ○ `SessionEnd` は文書化済み。1.0.5 の probe では teardown 時に user-level hook へ dispatch されたが、plugin 由来 hook（Traceary の経路）は dispatch されない | `traceary list --kind session_ended --limit 5` |
...
> **Grok Build 1.0.5 probe（2026-08-21、隔離 store）:** 新規の headless `grok -p` 実行と短い TUI セッション（private leader socket、使い捨て `TRACEARY_DB_PATH`、plugin 0.46.0 インストール・有効済み）で Traceary event は **0 件** でした（store ファイル自体が作成されず）。debug log では traceary-grok plugin が `has_hooks=true`・auto-trust（User scope）で discover されているのに、session の hook dispatcher が実行したのは global/user-level hook（`session_start`・`user_prompt_submit`・`stop`・`session_end` の `global/...` エントリ）のみで、plugin 由来 hook は両 mode で一度も実行されませんでした。host が配送しない event を adapter が capture することはできないため、plugin hook を dispatch する Grok Build リリースが確認できるまで上表の Grok cell は `available` とします。Traceary は `session_ended`（および他の event）をプロセス終了から合成しません。
