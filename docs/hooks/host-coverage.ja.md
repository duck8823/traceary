...
| `prompt` | ● `UserPromptSubmit` | ● `UserPromptSubmit` | ● `BeforeAgent` | ● `Stop`（直接の prompt field は無い。`transcriptPath` の最新 `USER_INPUT` / `USER_EXPLICIT` 行から復元） | ● `UserPromptSubmit`（`prompt` の content block 配列をテキスト化） | ○ plugin は `UserPromptSubmit` を配線済みだが、Grok Build 1.0.5 は headless `-p`・TUI の probe（2026-08-21）で plugin 由来 hook を一度も dispatch せず。最後の live capture は 0.2.99/0.2.101 | `traceary list events --kind prompt --limit 5` |
