...
| `compact_summary` | ● `PostCompact` (+ `PreCompact` marker, `SessionStart matcher=compact` resume) | ● `PreCompact` + `PostCompact` markers (`trigger` only; Codex exposes no compacted summary body) | ● `PreCompress` (marker only — Gemini exposes no post-compress hook with the resulting summary) | ✕ no documented compact hook | ● `PreCompact` + `PostCompact` (`trigger` = auto|manual marker with token counts; no summary body) | ● `PreCompact` + `PostCompact` (native live-observed markers from `source`; no summary body) | `traceary list events --kind compact_summary --limit 5` |
