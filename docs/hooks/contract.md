...
> **v0.21 note**: The successor host, Antigravity, is supported as a hook client from v0.21.1 (`PreInvocation` session start/refresh, `PreToolUse`/`PostToolUse` `run_command` pairing, `Stop` transcript/turn boundary) against the documented public hook/plugin surface. The Gemini CLI details above are legacy compatibility and are preserved for existing installs. See [Antigravity hooks and plugin](../integrations/antigravity.md).
