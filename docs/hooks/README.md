...
| Client | Settings file | Session start | Session end | Audit hook | Wake injection (#1684) | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| Claude Code | `.claude/settings.json` or `~/.claude/settings.json` | `SessionStart` | `SessionEnd` | `PostToolUse` + `PostToolUseFailure` with `matcher: "Bash"`, `matcher: "mcp__.*"`, and the built-in tool matcher (`Read\|NotebookRead\|Edit\|MultiEdit\|Write\|NotebookEdit\|Grep\|Glob\|Agent\|Task\|TodoWrite\|WebFetch\|WebSearch\|ExitPlanMode`) | `SessionStart` stdout (finished summaries only) | Anthropic's current docs define `Stop` as a per-response hook, not a session-end hook. |
| Codex CLI (`codex-cli 0.144.1`) | `~/.codex/hooks.json` | `SessionStart` | none (`traceary session end` / stale GC) | `PostToolUse` | `SessionStart` stdout (finished summaries only) | The official Codex CLI 0.144.1 hook reference exposes `SessionStart`, `SubagentStart`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `UserPromptSubmit`, `SubagentStop`, and `Stop`. No dedicated `SessionEnd`; `Stop` fires per assistant response so Traceary treats it as a turn-boundary transcript, not a session end (#1170). |
| Gemini CLI (`gemini-cli 0.36.0`) *(legacy compatibility)* | `.gemini/settings.json` or `~/.gemini/settings.json` | `SessionStart` | `SessionEnd` | `AfterTool` with `matcher: "run_shell_command"` | `SessionStart` stdout (finished summaries only) | Hooks are JSON-over-stdin / JSON-over-stdout and `SessionEnd` is best effort. Gemini CLI is the legacy path; Antigravity is the active successor (supported from v0.21.1). |
| Grok Build | packaged plugin hooks | `SessionStart` | none (stale GC) | `PostToolUse` | `SessionStart` stdout (finished summaries only) | Native `traceary hook grok session-start` preserves stdout for injection. |
| Kimi Code | packaged plugin hooks | `SessionStart` (ignored by host) | `SessionEnd` | `PostToolUse` | first `UserPromptSubmit` stdout | Kimi's `SessionStart` is fire-and-forget; wake injection runs once on the first prompt. |
| Antigravity (supported from v0.21.1) | `.agents/hooks.json` or `~/.gemini/config/hooks.json` | `PreInvocation` (no `SessionStart`) | none (`traceary session end` / stale GC) | `PreToolUse` + `PostToolUse` paired across `stepIdx` (`run_command` only) | out of scope (#1714) | `hooks.json` is a top-level hook-group map (not the shared `{"hooks": {...}}` shape). `Stop` is a per-execution turn boundary, not a session end (#1170). See [Antigravity hooks and plugin](../integrations/antigravity.md). |
...
**Wake injection (#1684).** On `SessionStart` (Claude / Codex / Gemini / Grok), Traceary may write **finished session summaries** for the same workspace to stdout so the host injects them into model context. Packaged wrappers already pass that stdout through (`preserve_stdout=1`). The channel carries only plain-text recalled summaries (never raw event bodies, never a bare session id). When there is nothing to inject, the hook writes **nothing at all**. Injection is at most once per session (resume / compact re-fires are no-ops). Kimi injects on the first `UserPromptSubmit` instead because its `SessionStart` is fire-and-forget. Antigravity wake injection is tracked separately as #1714. Budget: `wake_injection.budget_bytes` in [configuration](../environment/README.md).
...
Each hook publishes its current delivery under an in-flight filename, attempts it, and clears or promotes that file before it spends any remaining host timeout on backlog replay. Concurrent drainers cannot consume an active current delivery. Failure or cancellation atomically promotes it into the replay queue; an in-flight file abandoned by a killed host becomes replayable after one minute. Only after current success does the hook select and decode a bounded filename-ordered batch. A failed replay remains a v1 spool record but moves atomically behind records that have not yet been attempted, so one poisoned oldest record cannot starve later valid deliveries. `traceary doctor` reports retained or unreadable records through the `hook-spool` check; `traceary doctor --fix` drains selected records in rounds of at most 200, repeating rounds until the queue is empty or the shared 45-second fix wall is reached, and reports replayed, failed, and exact remaining counts. Inspect payloads under the spool directory before manual removal; Traceary does not delete records based on age alone.
...
Antigravity has no `SessionStart`: Traceary starts the session idempotently from `PreInvocation`. Like Codex, `Stop` is a per-execution turn boundary, so the session stays open and ends via `traceary session end` or stale GC. Only `run_command` tool calls are audited (the command args from `PreToolUse` are paired with the result from `PostToolUse`). See the [Antigravity hooks and plugin guide](../integrations/antigravity.md).
