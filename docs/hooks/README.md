...
| Antigravity (supported from v0.21.1) | `.agents/hooks.json` or `~/.gemini/config/hooks.json` | `PreInvocation` (no `SessionStart`) | none (MCP `manage_session` / stale GC) | `PreToolUse` + `PostToolUse` paired across `stepIdx` (`run_command` only) | out of scope (#1714) | `hooks.json` is a top-level hook-group map (not the shared `{"hooks": {...}}` shape). `Stop` is a per-execution turn boundary, not a session end (#1170). See [Antigravity hooks and plugin](../integrations/antigravity.md). |
