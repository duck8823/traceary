...
- `antigravity-hooks` — the aggregate summary. It `pass`es when **any** route is healthy and `warn`s with an actionable install message only when **no** route is healthy.
