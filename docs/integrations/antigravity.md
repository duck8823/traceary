...
- **`Stop` is a per-execution boundary, not a session end** (the same model as Codex — #1170). The session row stays open (memory auto-extract still fires) and ends only via `traceary session end` or stale GC (hook opportunistic GC / `traceary doctor --fix`).
