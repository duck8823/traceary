...
| `Stop` | assistant の transcript を session wire log（`session_index.jsonl` → `agents/main/wire.jsonl`）から best-effort で記録します。turn 境界であり、session 終了ではありません |
...
2. 対応する Traceary の release tag の checkout から、plugin をインストールします。
...
あるいは Kimi Code 公式の flow も使えます。Kimi Code 内で `/plugins install <path-or-url>` を実行し、`integrations/kimi-plugin/` またはこのリポジトリを指定してください。
...
- **transcript event が出ない**: transcript は session wire log から best-effort で復元します。`session_index.jsonl` にエントリがない古い session は設計どおり静かにスキップされます。
