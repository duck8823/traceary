...
| Grok Build | `integrations/grok-plugin/` | v0.23.0 でサポート。ネイティブ plugin は lifecycle hook 7 件を宣言し、共有 skill 4 件（[skills](./skills.ja.md)）を同梱します。宣言が上限です: Grok Build 1.0.5 は plugin 由来 hook を dispatch しないため（2026-08-21 probe で live capture 0 件）、`traceary doctor --client grok --json` は `grok-hooks` を完全検証済みの PASS ではなく WARN として報告します。`scripts/install-grok-plugin.sh` で導入し、`traceary doctor --client grok --json` で hook 契約、trust、パッケージ内容、バージョン一致を確認します。 |
