...
| Grok Build | `integrations/grok-plugin/` | Supported in v0.23.0. The native plugin declares seven lifecycle hooks, and the four shared skills (see [skills](./skills.md)). The declaration is the ceiling: Grok Build 1.0.5 dispatches no plugin-provided hooks (zero live capture in the 2026-08-21 probe), so `traceary doctor --client grok --json` reports `grok-hooks` as WARN rather than fully verified. Install it with `scripts/install-grok-plugin.sh`, then verify the installed hook contract, trust, inventory, and version parity with `traceary doctor --client grok --json`. |
