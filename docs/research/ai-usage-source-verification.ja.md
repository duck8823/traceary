...
- Codex capture mode は run ごとに開始時に固定する。Traceary-owned one-shot child は `headless_stream`、native/interactive は `rollout` を使う。両方が本文を含まない同じ排他キー `(thread_id, turn ordinal)` を導出する。そのキーで最初に永続化された observation が、単一の加算枠を原子的に取得する。後から到着した、または同時に到着した別 source は excluded evidence として保持する。通常の supervised execution では、legacy rollout scan より先に headless usage を記録する。
