...
- 一意な assistant request 1 件を provider response 1 件として扱う。transcript mode では一意な call だけを記録し、run summary は作らない。one-shot stream mode では assistant usage 行を無視し、最終 `result.usage` だけを run 利用量として記録する。2 mode は排他的な accounting 表現なので同じ run を二重計上しない。選択した mode の provider usage object がない abort/error は zero ではなく unavailable。
...
- status-line payload には provider request id、Stop の `executionNum`、`current_usage` と cumulative total の加算関係がない。Traceary は `idle` payload の `total_input_tokens` と `total_output_tokens` だけを immutable かつ非加算の session snapshot として保存する。新しい snapshot は以前の snapshot を supersede し、aggregate query は conversation ごとの最新 snapshot だけを選び、履歴を合算しない。`current_usage`、cache、call 単位利用量、価格は unavailable のままにする。
...
- `usage.record` には文書化された provider call id がない。#1452 は `(host, session_id, agent_id, usage-record ordinal)` を stable source identity とする。ordinal は append-only session wire 内の `usage.record` 行だけを数えるため、file を別の場所へ copy/replay しても変わらない。canonical payload fingerprint は identity ではなく衝突証拠として保存する。同じ identity と fingerprint は冪等、同じ identity で counter が違う場合は fail closed する。終端 record がなければ unavailable を維持する。
...
| Antigravity | immutable session snapshot を保存し、旧 snapshot を superseded にして、最新 cumulative input/output snapshot だけを aggregate する。`current_usage` は無視する。 | `idle` snapshot がない、または対応が曖昧なら partial/unavailable。 |
...
4. #1447: 排他的な Claude transcript-call または one-shot-run accounting と request/message 重複除外。
5. #1455: Gemini headless 終端 stats、非加算の Antigravity cumulative conversation snapshot、対話経路の明示的 unavailable。
