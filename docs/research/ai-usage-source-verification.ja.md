...
- Codex capture mode は run ごとに開始時に固定する。Traceary-owned one-shot child は `headless_stream`、native/interactive は `rollout` を使う。両方が本文を含まない同じ portable な排他キー `(thread_id, turn ordinal)` を observation に保存する。直列化した SQLite transaction と unique partial index により、そのキーで additive にできる observation は 1 件だけになる。後から到着した、import された、または同時に到着した別 source は excluded evidence として保持する。通常の supervised execution では、legacy rollout scan より先に headless usage を記録する。
...
- 現在のローカル probe は、導入済み individual Gemini client が非対応として拒否されたため実行できなかった。公式 output の分類は変えない。adapter は commit 済みの `presentation/cli/testdata/gemini_usage/v0.46.0/headless_stream.jsonl` contract fixture に固定し、不正または衝突する終端 metadata は安全側に倒して拒否する。
