...
> **Historical.** The operator cockpit (`traceary tui` / `traceary dashboard` and the bare TTY default that opened it) was removed in v0.35.0 (#1764) after the v0.34 deprecation window (#1687). Bare `traceary` always prints help. Use `traceary sessions --snapshot`, `traceary tail`, `traceary doctor`, and `traceary memory inbox review` for surviving interactive and script-friendly surfaces. The orphan local state file `~/.local/state/traceary/cockpit.json` (or `$XDG_STATE_HOME/traceary/cockpit.json`) is safe to delete manually.
