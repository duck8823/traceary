...
For `command_executed` rows, the event envelope body is empty by design:
the retained execution record is in `command_audits`. `store compact`
clears leftover historical bodies when an audit row exists (#1853).
Existing `command_audits.input_*` / `output_*` extent describes the
