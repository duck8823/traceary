...
- **CLI `--json` outputs** — `presentation/cli/testdata/<command>/<case>.golden.json`. Every new CLI `--json` flag must ship with a matching golden fixture in the same change. The covered representative surfaces include `event list` / `event search` / `event show`, `session start` / `session end`, `memory list` / `memory search` / `memory show` and the full `memory inbox` / `memory admin hygiene` family, `bundle import --json`, `timeline --json`, and `doctor --json`.
