...
- **CLI `--json` outputs** — `presentation/cli/testdata/<command>/<case>.golden.json`. Every new CLI `--json` flag must ship with a matching golden fixture in the same change. The covered representative surfaces include `event list` / `event search` / `event show`, `session list` / `session tree` / `session start` / `session end` / `session active` / `session latest`, `memory list` / `memory search` / `memory show` and the full `memory inbox` / `memory admin hygiene` / `memory admin graph` family, `sessions --snapshot --json` / `top --snapshot --json`, `bundle import --json`, `timeline --json`, and `doctor --json`.
...
- **MCP tool registry snapshot** — `presentation/mcpserver/testdata/tool_registry.golden.json`. A single fixture pins every registered MCP tool's name, description, annotations (`readOnlyHint` / `destructiveHint`), and the full inferred input and output schemas (property names, types, descriptions, and required fields). The test (`TestServer_ToolRegistrySnapshot`) lists tools through the in-memory MCP transport and sorts by name so the diff catches accidental adds / removes / renames as well as field-level drift inside either schema. `TestServer_ToolAdvertisementBudget` separately pins the compact response size and per-tool breakdown in `tool_schema_budget.golden.json`; see [MCP tool schema budget](./mcp-tool-schema-budget.md).

Starting in v0.30.0, CLI `traceary report --json` and MCP `get_report` serialize the same application `ReportSnapshot` read model. The report contract includes per-source aggregation extents; partial sources omit ratio fields instead of emitting misleading prefix-derived values. Starting in v0.32.0, that shared model also contains usage and deduplicated run-fact aggregates with explicit availability and cost-origin fields. CLI golden coverage and MCP handler tests must be updated together when that shared model changes.

`traceary sessions --snapshot --json` (and `traceary top --snapshot --json`, a compatibility alias deprecated in v0.34.0 and removed in v0.35.0) has a dashboard-specific contract that is intentionally separate from `traceary session tree --json`; the `latest_event_*` fields belong only to this dashboard snapshot contract. Starting in v0.14.0 the snapshot is wrapped in an envelope object with `sessions`, `failures`, `recent_commands`, and `candidates` (`{ count, items }`) keys so the snapshot mirrors the dashboard's secondary panes; consumers that previously read a bare top-level array of session nodes must read `sessions` from the envelope. Starting in v0.15.0 the envelope also includes `stale_memories` (`{ count, items }`) for the dashboard's stale-memory pane; stale rows reuse durable-memory summary fields plus a `reason`. Starting in v0.16.0, stale active session inspection can be enabled with `sessions --allow-stale` or `top --allow-stale`; stale session nodes then include optional `is_stale`, `stale_after_seconds`, and `stale_age_seconds` fields while existing session fields remain unchanged.

Starting in v0.19.0, the text snapshot for `traceary sessions --snapshot` (and `traceary top --snapshot`, a compatibility alias deprecated in v0.34.0 and removed in v0.35.0) includes `name="..."` before the raw `workspace=` / `agent=` metadata. This is covered by the text golden fixture; machine consumers that need positional stability should use the unchanged JSON envelope.
