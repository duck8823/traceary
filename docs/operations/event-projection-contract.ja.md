...
| audit metadataでも本文を読まない | CLI metadata fieldに`exit_code`を含む | list実行 | 1回のmetadata queryでexit codeを返し、event本文とcommand input/outputをSELECTしない | CLI/SQLite integration |
...
| #1433 CLI JSON | `--fields`指定でも`message`が出て、`exit_code`でdetail本文を読む失敗を固定 | fieldからmetadata query、任意audit metadata join、専用本文なしserializerへroute | list/tail互換経路のprojection resolverを共通化しN+1 extras hydrationを除く |
