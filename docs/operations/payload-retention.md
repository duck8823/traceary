...
Allocated bytes are an estimate of storage occupied by the selected payload or file, not a promise that the filesystem will immediately reclaim them. The raw-body plan stores each candidate’s non-negative encoded extent in `logical_extent.bytes`, reports the gross body-column total in `current`, and reports the sum of encoded retention-marker extents in `projected`. The operator-facing net change is `current - projected`; it may be zero or negative and becomes free disk space only after `store compact`. Candidate-level SQLite allocated bytes are not additive and cannot drive `raw_body` selection in v0.31; database allocated bytes are observation-only before/after compaction.
