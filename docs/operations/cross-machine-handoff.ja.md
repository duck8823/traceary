...
Traceary は `file_checksums` を使う v0.9.0 `manifest_version = 1` bundle も引き続き import できます。v2 は `tables` で table file を登録します。現在の writer は 5 テーブルすべて — `events`、`sessions`、`command_audits`、`memories`、`memory_edges` — を出力します。
