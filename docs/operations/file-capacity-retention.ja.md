...
- `apply` は実行者が所有していない root、または group/other user が書き込める root を拒否します。Traceary process 同士は root lock で協調します。同じ user の別 process が `.traceary-retention-*` name を意図的に変更している間は apply を実行しないでください。portable Unix には inode を条件にした unlink がないため、同じ user 権限で動く悪意ある process はこの local filesystem threat model の対象外です。
- `doctor --archive-root/--backup-root` は同じ descriptor に基づく境界を `apply_root=eligible`、`unsafe_owner`、`unsafe_permissions` として表示します。未対応・不明な根拠は警告し、適用可能とはみなしません。
- 削除では root の排他 lock、永続 catalog/journal/ledger、予約 tombstone への同一 directory 内 atomic no-replace rename を使います。移動後の tombstone は unlink 前に再検証し、競合した置換 file は original name が空いていれば復元して unlink しません。再試行は既知の状態だけを再開し、既存 tombstone を上書きしません。
