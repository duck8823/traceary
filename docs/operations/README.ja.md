...
- Codex CLI: host のセッション終了信号がない — `Stop` は応答ごとの turn 境界 (#1170) であり、Codex session は `traceary session end` または activity-aware stale GC（通常の hook start 後に自動実行、手動フォールバックは `traceary doctor --fix`）で終了する
...
`traceary store compact` がファイルを書き換え、領域をファイルシステムへ返します。
in-place `VACUUM` ではありません。旧 inode は受け入れるか
`store compact rollback RUN_ID` するまで残ります。
