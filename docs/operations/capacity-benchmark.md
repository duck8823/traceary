...
Each case is bounded by `--case-timeout` (default `2m`). A bounded failure is emitted as machine-readable `status: "timeout"` with `timeout_ms`; the report status is also `timeout`. The baseline validator accepts this as reproducible diagnostic evidence, but only `status: "passed"` is release-target performance evidence.
