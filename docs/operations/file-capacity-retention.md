...
- `apply` refuses a root not owned by the caller or writable by group/other users. Traceary processes coordinate through the root lock; do not run apply while another same-user process intentionally edits `.traceary-retention-*` names. A malicious process running as the same user is outside this local filesystem threat model because portable Unix has no conditional "unlink this inode" operation.
- `doctor --archive-root/--backup-root` reports the same descriptor-bound boundary as `apply_root=eligible`, `unsafe_owner`, or `unsafe_permissions`; unsupported/unknown evidence warns and never fabricates eligibility.
