...
Traceary still imports v0.9.0 `manifest_version = 1` bundles that use `file_checksums`. v2 registers table files through `tables`; current writers emit six table entries (`sessions`, `usage_observations`, `events`, `command_audits`, `memories`, `memory_edges`). `run_lineages` is a retired entry — legacy v2 bundles carrying it are checksum-verified and skipped when empty / atomically refused when non-empty on import, and new readers accept v2 bundles without that table.
...
Import verifies every registered file checksum before opening the write transaction, rejects unregistered payload files, and applies supported tables in dependency order across the current six-table portability surface:
...
| Legacy empty `run_lineages` entry | Skip after checksum/count validation | n/a | Rest of the bundle imports. |
| Legacy non-empty `run_lineages` entry | Reject | n/a | Atomic refuse before the write transaction; retrieve facts with the 0.48.2 binary. |
