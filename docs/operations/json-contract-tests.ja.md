...
v0.30.0 以降、CLI `traceary report --json` と MCP `get_report` は application の同じ `ReportSnapshot` 読み取りモデルを直列化します。レポート契約にはデータ源ごとの集計範囲が含まれ、部分集計では先頭部分から誤解を招く割合を出さず、該当フィールドを省略します。この共通モデルを変更する場合は、CLI golden と MCP handler test を同じ変更で更新してください。
