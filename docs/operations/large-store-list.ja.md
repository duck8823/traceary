...
- この command は data 削除、retention 適用、index build を行いません。read-only SQLite connection が WAL store を観測する際、一時的な WAL shared-memory sidecar を再作成することがありますが、Traceary の data mutation ではありません。capacity の回収は `traceary store compact` です。
