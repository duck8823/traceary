...
- **ストア管理** — `traceary store backup create`、`traceary store backup restore`、`traceary store compact`（`--archive`、`--archive-verify`、`--archive-restore`、`--retention-plan`、`--retention-apply` を含む）、`traceary store compact rollback`
