...
- `traceary store backup create` writes that file explicitly
- `traceary store backup restore` restores it to a Traceary DB path and reapplies migrations if needed
...
- **`traceary store backup create` output is safe to copy as a single file** — it is produced via `VACUUM INTO`, so it has no WAL sidecar to worry about
- **copying a live DB directly requires the WAL sidecars too** — the runtime DB uses `journal_mode=WAL`, which generates `<db>-wal` and `<db>-shm` files next to the main DB. External file-level tooling that snapshots only the `.db` file can produce an inconsistent copy; prefer `traceary store backup create` or include both sidecars in the copy
