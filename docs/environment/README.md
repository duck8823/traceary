...
| `consolidation.threshold_bytes` | integer | Stop-hook consolidation pressure threshold in body bytes. When unrefined session material (events strictly after the current refinement's `covers_to`, or the whole session when none exists) reaches this size, Claude / Codex / Kimi stop hooks exit 2 with a short reason on stderr so the agent can fold. Default **65536** (64 KiB) when the key or config file is absent. Explicit **0 disables** the trigger entirely. When the config file is present but unreadable or malformed, the trigger is also **disabled** (threshold 0) rather than defaulted — so a broken file cannot re-enable a trigger the operator turned off. Gemini, Antigravity and Grok never fire this check (they treat a non-zero stop exit as a plain failure; Grok has no continuing Stop surface). The request is issued at most once per session until a refinement lands, and never on a Stop whose payload carries `stop_hook_active: true` — the flag hosts set when the Stop is a continuation a hook itself caused. The reason names the `traceary-session-refine` skill and carries a runnable `traceary session refine … --covers-to … --produced-by agent` command. Any pressure-check failure (locked DB, missing session) fails open with exit 0. Independent of `wake_injection.budget_bytes`. |
| `wake_injection.budget_bytes` | integer | Maximum bytes of finished session summaries written to hook stdout at wake (#1684). Claude / Codex / Gemini / Grok inject on `SessionStart` stdout; Kimi injects on the first `UserPromptSubmit`. Default **8192** (8 KiB) when the key or config file is absent. Explicit **0 disables** injection entirely. When the config file is present but unreadable or malformed, injection is also **disabled** (budget 0) rather than defaulted — so a broken file cannot re-enable injection the operator turned off. Negative values are treated as disabled. Summaries are taken whole, newest first; if even one does not fit, nothing is written. Independent of `consolidation.threshold_bytes`. Antigravity is out of scope (#1714). |
