...
| `consolidation.threshold_bytes` | integer | Stop-hook consolidation pressure threshold in body bytes. When unrefined session material (events strictly after the current refinement's `covers_to`, or the whole session when none exists) reaches this size, Claude / Codex / Kimi stop hooks exit 2 with a short reason on stderr so the agent can fold. Default **65536** (64 KiB) when the key is absent. Explicit **0 disables** the trigger entirely. Gemini and Antigravity never fire this check (they treat non-zero stop exits as failures). Any check failure (locked DB, missing session, unreadable config) fails open with exit 0. |
