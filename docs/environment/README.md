...
| `consolidation.codex_prompt_only` | boolean | Opt-in Codex-only delivery mode. Default **false** preserves Stop exit 2. When **true**, a due Codex Stop records durable pending metadata and exits 0; the next Codex `UserPromptSubmit` emits exactly one bounded refinement action. It does not inject the ordinary prompt body, transcript, or previous summary. If no subsequent prompt arrives, the request stays pending; the operator can still run the normal manual refinement workflow. A failed pre-delivery write releases its lease and remains retryable. Delivery is confirmed only after stdout succeeds. A crash between stdout and confirmation is intentionally at-least-once: the durable lease expires and a later prompt can repeat the bounded action. Traceary does not claim control of host UI behavior. |
...
| `wake_injection.budget_bytes` | integer | Maximum bytes of finished session summaries written to hook stdout at wake (#1684). Claude / Codex / Gemini / Grok inject on `SessionStart` stdout; Kimi injects on the first `UserPromptSubmit`. Default **2048** (2 KiB) when the key or config file is absent. Explicit **0 disables** injection entirely. When the config file is present but unreadable or malformed, injection is also **disabled** (budget 0) rather than defaulted — so a broken file cannot re-enable injection the operator turned off. Negative values are treated as disabled. Summaries are taken whole, newest first; if even one does not fit, nothing is written. Set a larger explicit budget when more wake context is preferable to compact UI output. Independent of `consolidation.min_commands`. Antigravity is out of scope (#1714). |
