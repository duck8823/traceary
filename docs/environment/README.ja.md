...
| `consolidation.threshold_bytes` | integer | Stop hook の consolidation 圧力閾値（body のバイト数）。未精製のセッション素材（現行 refinement の `covers_to` より後のイベント、refinement が無い場合はセッション全体）がこのサイズに達すると、Claude / Codex / Kimi の stop hook が exit 2 と短い reason を stderr に出し、エージェントに畳みを依頼します。キー未指定時の既定は **65536**（64 KiB）。明示の **0 でトリガーを無効化**します。Gemini と Antigravity では発火しません（非ゼロ終了を失敗として扱うため）。判定失敗（DB ロック、セッション欠落、設定が読めない等）は常に fail-open で exit 0 です。 |
