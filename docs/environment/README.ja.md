...
| `consolidation.threshold_bytes` | integer | Stop hook の consolidation 圧力閾値（body のバイト数）。未精製のセッション素材（現行 refinement の `covers_to` より後のイベント、refinement が無い場合はセッション全体）がこのサイズに達すると、Claude / Codex / Kimi の stop hook が exit 2 と短い reason を stderr に出し、エージェントに畳みを依頼します。キーまたは設定ファイル未指定時の既定は **65536**（64 KiB）。明示の **0 でトリガーを無効化**します。設定ファイルが存在するが読めない、または JSON が不正な場合もトリガーは **無効化**（threshold 0）され、既定値には戻りません — 壊れた設定が、オペレータが明示的に切ったトリガーを再有効化しないためです。Gemini / Antigravity / Grok では発火しません（非ゼロ終了を単なる失敗として扱うため。Grok には継続する Stop 面がありません）。依頼は refinement が保存されるまでセッションごとに最大 1 回で、payload に `stop_hook_active: true` がある Stop — hook 自身が引き起こした継続であることをホストが示すフラグ — では発火しません。reason には `traceary-session-refine` skill 名と、実行可能な `traceary session refine … --covers-to … --produced-by agent` コマンドを含めます。圧力判定の失敗（DB ロック、セッション欠落）は常に fail-open で exit 0 です。`wake_injection.budget_bytes` とは独立です。 |
| `wake_injection.budget_bytes` | integer | 起床時に hook の stdout へ書く完成済みセッション要約の最大バイト数 (#1684)。Claude / Codex / Gemini / Grok は `SessionStart` stdout、Kimi は初回 `UserPromptSubmit` で注入します。キーまたは設定ファイル未指定時の既定は **8192**（8 KiB）。明示の **0 で注入を無効化**します。設定ファイルが存在するが読めない、または JSON が不正な場合も注入は **無効化**（budget 0）され、既定値には戻りません — 壊れた設定が、オペレータが明示的に切った注入を再有効化しないためです。負数は無効として扱います。要約は全文のまま新しい順に取り、1 件も入らなければ何も書きません。`consolidation.threshold_bytes` とは独立です。Antigravity は対象外 (#1714)。 |
