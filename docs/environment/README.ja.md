...
| `wake_injection.budget_bytes` | integer | 起床時に hook の stdout へ書く完成済みセッション要約の最大バイト数 (#1684)。Claude / Codex / Gemini / Grok は `SessionStart` stdout、Kimi は初回 `UserPromptSubmit` で注入します。キーまたは設定ファイル未指定時の既定は **8192**（8 KiB）。明示の **0 で注入を無効化**します。設定ファイルが存在するが読めない、または JSON が不正な場合も注入は **無効化**（budget 0）され、既定値には戻りません — 壊れた設定が、オペレータが明示的に切った注入を再有効化しないためです。負数は無効として扱います。要約は全文のまま新しい順に取り、1 件も入らなければ何も書きません。`consolidation.min_commands` とは独立です。Antigravity は対象外 (#1714)。 |
