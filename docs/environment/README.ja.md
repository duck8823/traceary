...
| `consolidation.codex_prompt_only` | boolean | Codex 専用の opt-in 配送モード。既定 **false** は Stop exit 2 を維持します。**true** の場合、due の Codex Stop は永続的な pending メタデータを記録して exit 0 になり、次の Codex `UserPromptSubmit` が境界付きの refinement action を 1 回だけ出力します。通常の prompt body、transcript、前回 summary は注入しません。配信前の書込み失敗は lease を解放して再試行可能です。stdout 成功後にだけ配信を確認します。stdout と確認の間で crash した場合は意図的に at-least-once です。durable lease が失効すると後続 prompt が境界付き action を再送できます。Traceary が host UI を抑制できるとは主張しません。 |
