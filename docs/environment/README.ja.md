...
| `consolidation.min_commands` | integer | Stop hook の consolidation 仕事量閾値。**main** セッション（`parent_session_id` なし、`subagent_kind` 空、`agent` に `/` なし）は、現行 refinement の `covers_to` より後（無い場合はセッション全体）の `command_executed` 件数がこの値に達すると畳みを依頼されます。キーまたは設定ファイル未指定時の既定は **20**。明示の **0 でトリガーを無効化**します。設定ファイルが存在するが読めない、または JSON が不正な場合もトリガーは **無効化**（0）され、既定値には戻りません。サブエージェントセッションは依頼しません。Claude / Codex / Kimi は exit 2 と短い reason を stderr に出します。Gemini / Antigravity / Grok では発火しません。`stop_hook_active: true` の Stop では依頼しません。判定失敗は fail-open で exit 0。`wake_injection.budget_bytes` とは独立です。 |
| `consolidation.stop_cadence` | integer | 同じセッションへの再依頼までに必要な、直前の consolidation request より後の `transcript` 件数。**最初の**依頼に窓は不要です。既定 **8**。明示の **0 で無効化**。壊れた設定は 0。ホスト allowlist・サブエージェント除外・`stop_hook_active`・fail-open は `min_commands` と同じです。`wake_injection.budget_bytes` とは独立です。 |
