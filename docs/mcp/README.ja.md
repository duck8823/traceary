...
現在の Traceary MCP server は 8 つの tool を公開します。
...
4. 新しい task の前に client から `get_context` を呼ぶ
5. 過去の command output や note が必要になったら `search` を呼ぶ
6. client 側で session lifecycle まで管理したい場合は `start_session` / `end_session` / `add_log` / `add_audit` を使う
