...
世代が完了し、前世代が回収された後、有界検索インデックスファミリの測定済み
アクティブ b-tree 割当は設定予算以下になります。この値は `dbstat` の割当であり
ファイルサイズではありません。ファイルは `store compact` で縮み、FTS5 は削除
ドキュメントの領域をセグメントマージ時にしか返しません。

**再構築中は保証されません。** `Start` は新世代が検証されるまで前世代を読める
