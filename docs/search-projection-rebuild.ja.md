...
`status.recent_oldest_norm` と `status.recent_newest_norm` は、**active** 世代
（`traceary search` が読む世代）の full-text recent ティアに実際に保持されている
`created_at_norm` の最古・最新値です。再構築中の世代がすでにより広い範囲を保持して
いても、検索はまだそこから answer できないため、その範囲は意図的に報告しません。
active ティアが空の場合（最初の世代がまだ complete していない store を含む）は
両フィールドを省略します。これは full-text の範囲だけを示します。この範囲外の
イベントも session ティア経由で引き続き検索できるため、範囲の開始が最近の日付でも、
それより古いものが検索できないという意味ではありません。
