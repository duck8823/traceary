...
`status.recent_oldest_norm` と `status.recent_newest_norm` は、status が示す世代の
full-text recent ティアに実際に保持されている `created_at_norm` の最古・最新値です。
ティアが空の場合は省略します。これは full-text の範囲だけを示します。この範囲外の
イベントも session ティア経由で引き続き検索できます。
