...
`status.recent_oldest_norm` と `status.recent_newest_norm` は、**いま
`traceary search` が読んでいる** full-text recent ティアに実際に保持されている
`created_at_norm` の最古・最新値です。読み取り経路と同じ条件（complete な世代で
あり、かつ active 世代として指名されていること）をそのまま適用します。したがって
検索がプロジェクションを読んでいない状態——再構築中、drift 後、最初の世代がまだ
complete していない store——では両フィールドを省略します。読み取り対象ティアが
空の場合も同様です。再構築中の世代がすでにより広い範囲を保持していても、それを
報告すると、広いクエリが候補を全件復号にフォールバックしているまさにその期間に
full-text の被覆を主張することになります。これは full-text の範囲だけを示します。
この範囲外のイベントも session ティア経由で引き続き検索できるため、範囲の開始が
最近の日付でも、それより古いものが検索できないという意味ではありません。
