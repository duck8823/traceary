...
（約 1.43 GiB）です。完了時のファミリ合計は
[プロジェクション再構築の完了リリースゲート](release/gates.ja.md#プロジェクション再構築の完了)
が live-store のコピー上で測り、記録します。この値でゲート判定はしません。
...
[プロジェクション再構築の完了ゲート](release/gates.ja.md#プロジェクション再構築の完了)
はイテレーションごとに `db_bytes_after` と空き容量を記録するので、将来のピーク測定の起点になります。`disk_full` での停止は再試行せず記録します。
