...
func TestCurrentReaderVersionIsTheRaisedGate(t *testing.T) {
	t.Parallel()
	if currentReaderVersion != 42 {
		t.Fatalf("currentReaderVersion = %d, want 42", currentReaderVersion)
	}
	const previousReader = 41
...
func TestVerifyStoreCompatibilityRejectsReaderBelowMinimum(t *testing.T) {
	...
	// The live binary speaks 42, so min=39 is accepted. A 41 binary would
	// compare 42 > 41. Pin that inequality here; the error string is covered
	// by the future-reader tests with min=43.
	if previous != 42 || 42 <= 41 {
		t.Fatal("previous reader 41 would not be rejected by min=42")
	}
