...
func TestWithReadScope_CompatibilityFailureStopsAtEntry(t *testing.T) {
	...
	if _, err := raw.Exec(`CREATE TABLE store_format_state(singleton INTEGER PRIMARY KEY, minimum_reader_version INTEGER NOT NULL); INSERT INTO store_format_state VALUES(1, 41)`); err != nil {
		t.Fatal(err)
	}
	if err := raw.Close(); err != nil {
		t.Fatal(err)
